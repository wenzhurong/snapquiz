...
>
> **实现快照**：M0–M4、M5/W08 与 W09 的 local/offline 范围已完成并推送至 `main@341374b`；该提交保留 W09 双 Python 1235/1235、B4 7/7、native build 16/16、numeric/TLS/transfer 112/112 以及三轮 P0=0/P1=0 的离线证据。当前未提交 W10 增量新增 Provider-neutral `DirectMultimodalAdapter`、唯一 test-authorized executor、caller-preheld context/credential/resolver/cleanup owners、capture-start/result-publication 线性化 claim、取消重驱、single-flight 与 cleanup-only finalization；Egress trusted Adapter 选择已改为目录驱动，不再在核心校验中写死单一 Provider 类。Python 3.12.13 与 3.10.20 的 W08–W10 定向安全集分别 341/341 通过，完整离线 suite 分别 1341/1341 通过；独立终审为 P0=0、P1=0、P2=0。所有 production flags 仍为 false；没有真实截图、secret、DNS/socket/HTTP、Provider API、应用入口接线或部署证据。
>
...
- phase1 pass-through EgressGate 在生成预览前用 trusted Adapter 独立重建一次 `PreparedOutbound` 并逐 slot/逐 body byte 比较，绑定 request/plan/planned execution/registry/privacy authorization/stage/operation/invocation/source/scope/exact envelope；预览返回后只重算并复核完整 review subject 与原始 exact bytes，不再次调用 Adapter；自洽 alternate body、预览期间 mutation 或 capture release 都 fail-closed；
...
因此 M5 当前为 production `blocked`：W09-B2a 的 owner-bound AttemptGate、纯地址策略、offline coordinator、helper/RESULT wire v2、completion/recovery/deadline 合同，以及 B2b S1–S6、caller-preheld native resolver/output owner、Keychain bridge、opaque numeric/TLS owner、原生 C-to-C transfer、B3 fake-edge exact Transport 与 B4/readiness v5，均已作为 local/offline 范围提交并推送至 `main@341374b`。上述证据使 W09 的 local/offline 与无生产身份可构建范围 `complete`；production 仍需外部身份、最终签名 artifacts、真实系统 vtable/entitlement 接线与真实 macOS liveness 授权，不能把 production-unwired native foundation 或 fake-edge B3/B4 当作 production completion。Phase 1 Transport 仍固定走自有 exact 单请求 HTTP/1.1，禁止 SDK/httpx 隐式行为、自动 retry、redirect、连接池复用、HTTP/2、proxy、chunked request 与透明压缩；任何额外 HTTP 请求都必须是新的获批 attempt。
...
真实 helper external-action uncertainty 仍是诚实的不可同步恢复边界：一旦 ledger 为 `cleanup_failed`，ticket 必须保持 recoverable/nonterminal，后续 retry 不得重放 terminate/reap/close，Attempt 与 secret owner 继续保留；这里的 recoverable/retryable 只表示 cleanup-only owner 仍可安全调用，不保证状态能推进，每次仍可能返回 false。只有资源 action 已证明完成、仅 `finish_cleanup` bookkeeping no-op 时，retry 才能补做终态 bookkeeping 并继续清 Gate/credential。三层独立 observer 全部通过后先不可逆记录 `resources_terminal_proven`；若随后 strong-ref release 或其 observer 失败，后续 retry 只补引用释放/terminal bookkeeping，不再运行 helper、Attempt 或 credential cleanup。只有 terminal proof 与全部聚合引用释放 postcondition 都成立，ticket 才可转为 terminal。B2a 最终的 helper monotonic deadline/cancellation 离线切片已经完成并验证，故 W09-B2a 为 `complete`。B2b S1–S6、native resolver/output、Keychain、numeric/TLS 与 C-to-C transfer foundations、B3 fake-edge exact Transport 及 B4/readiness v5 也已随 `main@341374b` 推送；这些模块没有绑定 fixed Team-signed artifacts、真实系统 vtable/entitlement 或 app authority，因此 W09-B2、W09-B3、W09-B4 与 M5 的本地范围 `complete`、production 范围 `blocked`，S7 保持未启用。
...
- B2a 默认验收只使用 injected fake spawner/kernel，并以 poison process/DNS/socket/network 固定零真实 I/O；native executable/spawn shim、真实 process/pipe/poll/kill/waitpid/reap/FD liveness、DNS、numeric socket/connect 与 peer/rebinding 全部属于 W09-B2b，production spawner 继续零进程 fail-closed。B2a 切片通过完整离线 580/580、W09 九模块 275/275、核心五模块 198/198 及独立终审，因此 W09-B2a 为 `complete`；`main@341374b` 随后完成 B2b S1–S6 及其可在无生产身份下构建的 native owner foundations，整个 W09-B2 与 M5 的 production 范围则因外部门槛为 `blocked`。

W09-B2b 在远端 `main@341374b` 的状态为 **local/offline `complete`、production `blocked`**：S1、S2a、S2b-I1、S2b-I2、S2c、S3、S4、S5、S6 的 local/offline foundations 已实现并通过双版本全量与独立终审，但 production wiring 与 S7 未完成：
...
### W10：direct multimodal pipeline（local/offline complete）

W10 在不启用任何 production edge 的前提下，把 W05–W09 的既有 authority 组合为唯一的测试授权执行链。它不是第二套权限、密钥、DNS、重试或 HTTP 实现，也没有连接 CLI/app；`MultimodalPipelineExecutor.production()` 与 `PRODUCTION_MULTIMODAL_PIPELINE_AVAILABLE` 继续 fail-closed。

当前实现固定以下顺序和所有权边界：

1. 先验证 request/plan/scope，再由 `RuntimeCallFactory` 发布 exact `CallContext`；只有账本独立证明 authorization/context/cancellation source/start owner 全部属于本请求后，executor 才采纳返回值；
2. 每个 authority checkpoint 都先重驱一次已锁存的取消请求，再由同一 Context 复核 monotonic deadline、Registry/Policy lease 与 cancellation；CapturePolicy 返回的 authorization 与 consumption 都必须由原 capture ledger 以 exact identity/digest 独立证明，随后把 validation recovery 所需的 ledger+consumption 作为单一 holder 原子发布；再取得独立可观察的 capture-start claim，才允许唯一一次同步 capture；
3. 捕获后重新验证 permission/topology/PNG；`InputValidator` 的正常返回不能自行授权下游，只有原 capture ledger 以同一对象 identity 与 validation digest 证明 exact `ValidatedCapture` publication 后，才能形成 `SolveRequest → StageInvocation → PreparedOutbound`；这两个纯 factory 返回还必须复核同一 active lease identity 与完整 digest 链，不能用另一个合法 request/invocation 替换输入。caller Adapter 只 prepare 一次；Egress 为防止伪造再从受信 Adapter 目录独立重建一次，预览后只重算完整 subject，不再第三次 prepare；
4. EgressApproval/Session/Attempt/Resolver 全部沿既有账本签发。credential permit publication、resolver cleanup ticket 与 coordinator 返回的 `PreparedResolverAttempt` 都在下一风险动作前预持 owner 并用独立 observer 复核；返回对象还必须同时绑定当前 ticket、launcher、resolver、gate、credential permit、context、session、operation 与 envelope，foreign/alias 返回在 secret 之后但 Transport 之前 fail-closed，且不得被当前 executor 接管或关闭；
5. Transport 只能执行 `send_once`，不含自动 retry。response、candidate 与最终 `SolveResult` 逐层复核 request/plan/stage/operation/invocation/envelope/body digest；validator 返回的结果还必须与同一 candidate payload 和 executor 构造的 provenance 逐字段等价，return alias 不能改写答案。只有 result-publication claim 在线性化竞态中获胜，结果才可交付；
6. 所有成功、失败、取消和超时都按 Prepared/ticket → credential permit/Gate → ValidatedCapture → Context → cancellation binding 的 owner 顺序清理。validation recovery 的 ledger+consumption 使用单一原子 holder，避免异步中断落在两次状态写入之间形成永久 pending；Context 未返回时的 absence proof 只针对本次预持的 `start_owner`，同一 `request_id` 下合法的 foreign Context/pending start 不得被采纳、关闭或阻塞本次 finalization，畸形或 owner 不一致状态仍 fail-closed。若外部资源终态或 executor bookkeeping 不能证明，保留 cleanup-only recovery state；`retry_cleanup_and_finalize()` 只能清理并交付原先已确定的结果/错误，绝不重放截图、secret、resolver、Transport 或模型解码。

W10 故障矩阵覆盖首个权限 observation 的 denied/unknown、捕获异常/非法 frame、捕获后权限或 topology 变化、ValidatedCapture publication/return-loss/foreign alias、Adapter 异常/非法 envelope、预览取消/异常、Session 与 credential publication、credential read、resolver 非法结果、foreign Prepared 返回、resolver observation 与 transport claim 之间 cleanup 先赢、Transport timeout/correlation、candidate/result validation、取消/deadline、authority revoke、账本 partial/no-op/commit-then-raise、上下文/返回 alias、single-flight 与 cleanup retry。Python 3.12.13 与 3.10.20 的 W08–W10 定向安全集分别 341/341 通过，完整离线 suite 分别 1341/1341 通过，`compileall` 与 `git diff --check` 通过；独立终审为 P0=0、P1=0、P2=0。测试只使用合成 PNG、fake credential source、fake resolver/kernel 与 fake numeric/TLS edge；没有读取真实 key、捕获真实屏幕、执行真实进程/DNS/socket/HTTP 或调用 Provider API。

W10 的 `complete` 仅指 local/offline 组合与安全证据完成。它不关闭 W09 production blockers，不表示存在用户可达解题路径，也不授权 W11 live smoke、W12 真实截图、合并、部署或发布。
...
| W10 | 新 multimodal pipeline（local/offline complete；production unavailable） | W06,W07,W09-B4 local/offline | `pipelines/multimodal.py`、`tests/test_w10_*` | 唯一 gate 顺序、返回 owner attestation、故障/竞态/清理矩阵；零真实 I/O |
...
当前工作包状态：W01–W09 的 local/offline 范围 complete，远端基线为 `main@341374b`。当前未提交工作区已完成 W10 的 local/offline executor、Provider-neutral Adapter interface、exact owner publication/return attestation、取消/竞态/cleanup recovery 与端到端故障矩阵；Python 3.12.13 与 Python 3.10.20 的 W08–W10 定向集分别 341/341、完整离线 suite 分别 1341/1341 通过，独立终审为 P0=0、P1=0、P2=0。Production W09/M5 仍阻塞于 Team-signed 最终 artifacts/bundle facts、真实 process/DNS/socket/SSL/Security.framework vtable 与 Keychain entitlement、真实 control/liveness/transport evidence、S7/app cutover 及另行授权的 activation/acceptance；W10 production factory 也保持 disabled。故 W10 complete 只表示本地/离线组合完成，不表示 Provider、真实 macOS 捕获、传输或发布链可用。
...
1. **已完成并推送** W01–W09 的 local/offline 范围，远端基线为 `main@341374b`；W09 production 范围保持 blocked；
2. **W10 local/offline complete，当前未提交**：唯一 direct-multimodal executor、gate 顺序、capture/result claim、owner return attestation、取消/single-flight/cleanup recovery 与完整离线负向矩阵均已实现；双 Python 定向集分别 341/341、完整离线 suite 分别 1341/1341、`compileall` 与 diff 检查均通过，独立终审 P0/P1/P2=0；
...
4. W10 可以并且已经基于 W09 local/offline foundations 完成；W09 production closure 不阻塞 W10 的离线软件工作，但它仍是任何 W11 真实调用的硬前置条件；
...
- W09 剩余 local/offline 增量已经提交并推送为 `341374b feat: complete W09 transport foundations`：B2b S1–S6、caller-preheld native resolver/output owner 与 async adapter、Keychain staged source→CredentialResolver bridge、opaque numeric/TLS owner 与原生 C-to-C descriptor transfer、B3 exact TLS/HTTP/1.1 + fake-edge single-request Transport、reproducible native build，以及 B4 fault/race/cleanup + readiness v5 exact exit inventory 均已落地。该提交保留双 Python 完整离线 suite 各 1235/1235 与三轮 P0=0/P1=0 的证据；Production W09/M5 仍需 fixed Team-signed artifacts、最终身份/entitlement、真实系统 vtable/evidence、S7/app cutover 与独立授权，所有 production flags 保持 false。
- 当前未提交 W10 增量完成唯一 direct-multimodal test executor、Provider-neutral Adapter 基类、capture-start/result-publication claim、caller-preheld context/credential/resolver recovery、foreign return attestation、取消重驱、single-flight、结果 correlation 与 cleanup-only finalization，并扩展 Egress/Session/Attempt/Context 的 no-op、partial、commit-then-raise 与 alias 回归。W08–W10 双 Python 定向集分别 341/341、完整离线 suite 分别 1341/1341 通过，独立终审 P0/P1/P2=0；没有真实截图、secret、进程、DNS/socket/HTTP 或 Provider API。用户已有 `README.md` 改动继续保持未暂存且未被修改。

...
> **状态**：Active
>
> **实施基线**：`2026-08-31`，W09-B2a 第一批离线安全原语已提交并推送至 `main@62c53c1`；当前 B2a coordinator/RESULT v2 延续仍在工作区，尚未提交
>
> **实现快照**：M0–M4、M5/W08、W09-A、W09-B0 与 W09-B1 已完成；W09-B2a 整体仍为 `in_progress`。远端 `main@62c53c1` 已包含 caller-owned `io_claimed`、terminal-guard/DNS START proof、冻结地址策略、`ResolutionSet`/pure peer matcher 与 injected helper lifecycle 第一批离线原语。当前未提交工作区新增唯一 offline coordinator、helper/RESULT wire v2、ledger-issued RESULT receipt、单次 ledger-anchored `ResolutionSet`，以及 credential/READY 的 per-call return-publication recovery；production helper 仍零进程 fail-closed。B2a 仍缺 factory-generated READY/lifecycle proof 与 public bypass 封口、deadline/cancellation、RESULT 后 EOF/无第二帧/child exit-zero/reap success attestation、cleanup-failure recovery ticket/retry；B2b native process/DNS/numeric connect/peer、B3 exact TLS/HTTP 与 B4 全资源矩阵也未实现，因此仍没有可发送 Transport。用户已有的 `README.md` 改动保持未暂存且未被本轮修改。应用仍被有意冻结，没有真实截图、真实人机预览、出站传输或可执行解题链。
>
...
| M5 | in_progress | Egress、session、Transport、预算、取消、授权租约、延迟密钥与统一清理 | W08、W09-A、W09-B0、W09-B1 与 B2a 第一批离线原语已推送；B2a coordinator/receipt/ledger binding 当前未提交且仍缺完整 lifecycle/cancellation/attestation/recovery；B2b–B4 DNS/peer/exact HTTP/TLS/全资源 cleanup 尚未实现 |
...
因此 M5 仍为 `in_progress`：W09-B2a 第一批 owner-bound AttemptGate、纯地址策略与 injected helper lifecycle 离线原语已进入远端 `main@62c53c1`；当前未提交工作区新增唯一 offline coordinator、helper/RESULT wire v2、ledger-issued receipt、单次 ledger-anchored `ResolutionSet`，以及 credential/READY 的 per-call return-publication recovery。W09-B1 complete 只表示延迟 secret resolve、handle proof、owner-only one-shot borrow 与凭据终态合同成立；当前 B2a 也仍只表示 fake-kernel 下的组合顺序、proof binding 与返回值清理 ownership 成立。public launcher/guard primitive 尚可绕过 coordinator，READY/lifecycle proof 仍非 factory-generated capability；B2a 还缺贯穿 helper 的 deadline/cancellation、RESULT 后 EOF/无第二帧/child exit-zero/reap success attestation 与 cleanup-failure recovery ticket/retry。W09-B 另缺 cancellable production DNS/native strict-spawn、numeric connect、TLS/hostname、精确单请求 HTTP/1.1 `send_once`、真实连接 peer/rebinding、redirect 禁止、bounded response 与完整 I/O cleanup。Phase 1 Transport 固定走自有的 exact 单请求 HTTP/1.1 路径，不使用 SDK/httpx 的隐式行为；禁止自动 retry、redirect、连接池复用、HTTP/2、proxy、chunked request 与透明压缩，任何额外 HTTP 请求都必须是新的获批 attempt。
...
当前未提交延续新增 `transport/http.py` 的唯一 offline coordinator，固定 READY → credential resolve → attempt reserve/claim → helper transfer/guard bind → DNS START commit → single START/RESULT 顺序，并返回 factory-only `PreparedResolverAttempt` 作为 B2b 后续接点。helper wire 已升级为 v2；terminal guard 只发行 factory-only `ResolverResultReceipt`，ledger 独立保存 exact START frame、原始 transcript 与 receipt 快照，并把每个 receipt 的单次 `ResolutionSet` 及 candidates identity/digest/canonical payload 锚定到同一发行事实。credential handle recovery 必须匹配 resolver/Gate 两侧的 per-call publication ID；READY reservation 在调用前取得 owner identity，ticket 在 spawn 前预占并 exact consume/recover，reservation 自身和 guard 返回的 return-then-raise 窗口均只能回收本次 owner。它们只用于清理，不授予新发送权限。完整离线 suite 为 466/466，W09 为 161/161，当前共 102 个 Python 文件，本机仍为 Python 3.12.13。B2a 仍须先补 RESULT 后 stdout EOF/无第二帧/child exit-zero/reap success attestation，再把 READY/lifecycle/owner proof 改为 factory-generated capability 并封住 public launcher/guard bypass，同时补贯穿 helper 的 monotonic deadline/cancellation，以及 cleanup fault 后可达且可重试的 recovery ticket；当前 failure path 尚不能称为 recovery anchor 已完成。B2b 再补 native helper/spawn shim、真实 process/pipe/select/kill/reap/FD liveness、DNS、numeric socket/connect/getpeername；B3 exact HTTP/TLS 与 B4 全资源矩阵仍待完成。当前不包含真实进程、DNS/socket/HTTP 或 Provider API，因此 W09-B2/M5 继续保持未完成。
...
| W09-B2a | owner、地址策略与 helper 离线生命周期 | W09-B1 | `runtime/attempt.py`、`transport/address_policy.py`、`transport/resolver.py`、`transport/http.py` | `io_claimed` owner/guard、READY/transfer/single START、16 KiB/32 条整组地址策略、唯一 offline coordinator、ledger-issued RESULT/Resolution binding、factory capability/cancel/EOF-exit-reap attestation/recovery、production fail-closed |
...
当前工作包状态：W01–W09-B1 complete，W09-B2a 第一批离线原语也已推送至 `main@62c53c1`；整个 W09-B2a 仍为 `in_progress`，当前 coordinator/receipt/ledger binding 延续尚未提交，W09-B2b–B4 与 W10–W15 为 `pending`。当前增量只表示 fake source/kernel 下的唯一组合顺序、proof binding 与 return-publication ownership 存在，仍不表示 production credential source、factory capability 封口、deadline/cancellation、EOF/exit/reap success attestation、cleanup recovery、resolver/process、DNS/numeric connect/HTTP Transport、Provider、真实 macOS 捕获、传输或发布链可用，因此 W09-B2 与 M5 保持 `in_progress`。
...
1. **已完成并推送** M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 第一批离线安全原语，远端基线为 `main@62c53c1`；
2. **当前工作区已实现、尚未提交** W09-B2a 延续：唯一 offline coordinator、helper/RESULT wire v2、ledger-issued receipt/单次 `ResolutionSet`、per-call credential/READY publication recovery，以及 exact claim/guard/DNS START/target/policy/transcript/candidate binding 已通过 fake source/kernel 离线矩阵；
3. **下一步**先补 RESULT 后 EOF/无第二帧/child exit-zero/reap success attestation，再用 factory-generated READY/lifecycle capability 封住 public bypass，并补贯穿 helper 的 deadline/cancellation 与 cleanup-failure recovery ticket/retry；随后实现 W09-B2b native strict-spawn production resolver、真实 process/pipe/select/kill/reap/FD liveness、numeric single-connect 与 peer/rebinding proof，再完成 B3 exact Transport 与 B4 全资源矩阵；
...
- 继续推进 W09-B2a 未提交延续：新增唯一 offline coordinator 与 factory-only `PreparedResolverAttempt`，将 READY、secret resolve、budget reserve、attempt claim、helper ownership、DNS START、RESULT parsing 强制成单一离线顺序；升级 helper/RESULT wire v2，新增 ledger-issued receipt、单次 ledger-anchored `ResolutionSet`、per-call credential publication ID 与 caller-owned pre-spawn READY reservation/ticket，exact 绑定 claim、terminal guard、DNS START/START frame、canonical target/policy、原始 transcript/candidates，并拒绝 active/unclaimed attempt、terminal receipt、跨 proof replay、跨调用 owner recovery 与对象内摘要重算。完整离线测试 466/466、W09 161/161，当前 102 个 Python 文件；production spawner 保持零进程 fail-closed。factory-generated READY/lifecycle proof 与 public bypass 封口、deadline/cancellation、RESULT 后 EOF/无第二帧/child exit-zero/reap success attestation、cleanup-failure recovery ticket/retry、B2b native process/DNS/numeric connect/peer、B3 exact HTTP/TLS 与 B4 全资源矩阵仍待完成，本增量尚未提交。

...
> **状态**：Active
>
> **实施基线**：`2026-09-02`，W09-B2a capability/recovery sealing 已提交并推送至 `main@5468b52`；当前 caller-owned `ResolverCleanupTicket` 聚合恢复增量仍在工作区，尚未提交
>
> **实现快照**：M0–M4、M5/W08、W09-A、W09-B0 与 W09-B1 已完成；W09-B2a 整体仍为 `in_progress`。远端 `main@5468b52` 已包含 caller-owned `io_claimed`、terminal-guard/DNS START proof、冻结地址策略、injected helper lifecycle、唯一 offline coordinator、ledger-issued RESULT receipt/`ResolutionSet`、completion attestation，以及 factory-issued lifecycle capability、spawn-before-return kernel publication、launcher strong recovery registry、cleanup-only ledger snapshots/state recovery 与 public bypass sealing。当前未提交工作区新增在任何 reservation/spawn/secret read/budget 前签发的 caller-owned `ResolverCleanupTicket`：同 ticket 只能绑定一次 coordinator，失败或 Prepared 返回被外层丢失后仍可按 helper → Attempt/Gate → credential 的固定顺序重试 cleanup；recovery 返回值不能冒充终态，三层必须分别由独立 observer 证明。`publish_recoverable` 是 Prepared 返回前的 cleanup ownership 线性化点，之后显式并发 retry 可以赢，返回对象会同步为 closed。三层终态确认后先记录 `resources_terminal_proven`，strong-ref release 或其 observer 失败只重试 bookkeeping，只有引用也独立证明已释放后才提交 terminal。`retry_cleanup()` 在 issued/coordinating 时不执行清理，不是 cancellation；瞬时 cleanup wrapper fault 返回 false 并恢复 recoverable，并发 retry 被串行化，且不能触发 resolve/reserve/claim/START/RESULT/borrow。helper 外部 action 结果不确定时 ticket 会诚实保持 recoverable/nonterminal，绝不重放 terminate/reap/close；该状态只表示 cleanup owner 仍可安全调用，不承诺重试一定推进。当前完整离线 suite 为 557/557，W09 为 252/252，核心五模块为 197/197，coordinator 为 82/82；101 个 Python 文件通过 3.10 AST、`compileall`、依赖方向与 diff 检查。production helper 仍零进程 fail-closed。B2a 现在只剩贯穿 helper 阻塞点的 monotonic deadline/cancellation；B2b native process/DNS/numeric connect/peer、B3 exact TLS/HTTP 与 B4 全资源矩阵也未实现，因此仍没有可发送 Transport。用户已有的 `README.md` 改动保持未暂存且未被本轮修改。应用仍被有意冻结，没有真实截图、真实人机预览、出站传输或可执行解题链。
>
...
| M5 | in_progress | Egress、session、Transport、预算、取消、授权租约、延迟密钥与统一清理 | W08、W09-A、W09-B0、W09-B1 与 B2a capability/recovery sealing 已推送；caller-owned cleanup ticket 当前未提交，helper deadline/cancellation 与 B2b–B4 DNS/peer/exact HTTP/TLS/全资源 cleanup 尚未实现 |
...
因此 M5 仍为 `in_progress`：W09-B2a 的 owner-bound AttemptGate、纯地址策略、injected helper lifecycle、唯一 offline coordinator、helper/RESULT wire v2、ledger-issued receipt/`ResolutionSet`、completion attestation，以及 capability/recovery sealing 已进入远端 `main@5468b52`。launcher 已在 spawn 前 factory-generate READY publication、lifecycle、transport claim 与 DNS START proof，并以 capability digest、exact lifecycle capability identity 和 ledger-owned snapshot 绑定；spawn kernel 必须先发布进 lifecycle ledger 才能从 spawner 返回，launcher 强引用 recovery registry 保留 cleanup-failed ledger/kernel。reservation、credential handle、attempt permit、terminal guard 与 Prepared cleanup 的正常返回别名/篡改窗口只走 exact owner 的 cleanup-only recovery；所有安全敏感 transition 的 normal return 还需独立 ledger postcondition，no-op 会被拒绝并回滚或经另一条 state recovery 精确终结，borrow owner 未提交时绝不开放 secret view；终态同时清掉 Attempt/Credential state 的 context/recovery refs。helper 的 `finish_cleanup` 只负责终态 bookkeeping：若它 no-op，已证明的外部资源动作不重放，ledger 保持可重试，strong registry 也保持到 exact terminal。公开 coordinator 不再接受三个 proof UUID，旧 public launcher/transfer/start/result-read 入口不存在，私有入口缺少 coordinator authority 时在任何外部动作前拒绝，reservation 状态原子执行 `reserved → launching → published → consumed`，因此同 lifecycle capability 并发只能 spawn 一次。当前未提交的 caller-owned cleanup ticket 在上述内部恢复之上补齐失败后的调用方可达性，但不把 `cleanup_failed` 谎报为 terminal/cleaned；recoverable/retryable 只表示 cleanup-only owner 仍可安全调用，不保证重试能推进。W09-B1 complete 只表示延迟 secret resolve、handle proof、owner-only one-shot borrow 与凭据终态合同成立；当前 B2a 仍只表示 fake-kernel 下的组合顺序、proof binding、返回值清理 ownership、completion attestation 与项目内防误用 capability 边界成立。B2a 现在只缺贯穿 helper 的 deadline/cancellation；W09-B 另缺 cancellable production DNS/native strict-spawn、numeric connect、TLS/hostname、精确单请求 HTTP/1.1 `send_once`、真实连接 peer/rebinding、redirect 禁止、bounded response 与完整 I/O cleanup。Phase 1 Transport 固定走自有的 exact 单请求 HTTP/1.1 路径，不使用 SDK/httpx 的隐式行为；禁止自动 retry、redirect、连接池复用、HTTP/2、proxy、chunked request 与透明压缩，任何额外 HTTP 请求都必须是新的获批 attempt。
...
| W09-B2a | owner、地址策略与 helper 离线生命周期 | W09-B1 | `runtime/attempt.py`、`transport/address_policy.py`、`transport/resolver.py`、`transport/http.py` | `io_claimed` owner/guard、READY/transfer/single START、16 KiB/32 条整组地址策略、唯一 offline coordinator、ledger-issued RESULT/Resolution binding、factory capability/kernel prepublication/EOF-exit-reap attestation、caller-owned aggregate cleanup ticket/retry、production fail-closed；deadline/cancel 留下一切片 |
...
当前工作包状态：W01–W09-B1 complete，W09-B2a capability/recovery sealing 已推送至 `main@5468b52`；整个 W09-B2a 仍为 `in_progress`，caller-owned aggregate cleanup ticket/retry 当前尚未提交，W09-B2b–B4 与 W10–W15 为 `pending`。当前增量只表示 fake source/kernel 下的 factory role proof、single-spawn、exact-owner recovery、EOF/无第二帧/exit-zero/single-reap/pipe-close 发行门槛，以及失败后调用方仍持有 cleanup-only owner；稳定证据为完整离线 557/557、W09 252/252、核心五模块 197/197、coordinator 82/82。它不表示 `cleanup_failed` 一定能恢复，更不表示 production credential source、helper deadline/cancellation、resolver/process、DNS/numeric connect/HTTP Transport、Provider、真实 macOS 捕获、传输或发布链可用，因此 W09-B2 与 M5 保持 `in_progress`。
...
1. **已完成并推送** M5/W08、W09-A、W09-B0、W09-B1，以及 W09-B2a 的 owner/address/helper、唯一 offline coordinator、RESULT receipt/`ResolutionSet` ledger binding、completion attestation 与 capability/recovery sealing，远端基线为 `main@5468b52`；
2. **当前工作区已实现、尚未提交** W09-B2a caller-owned recovery ticket：`issue_resolver_cleanup_ticket()` 在任何 resolver 风险动作前签发 cleanup-only owner，coordinate 必须 exact bind 一次；聚合 ledger 冻结 helper、Attempt/Gate、credential 三层 proof，按固定顺序重试并以独立 observer 判定终态。Prepared return 丢失、factory attach/bind/record commit-then-raise、normal no-op、并发 publication/retry、瞬时 cleanup wrapper/ref-release observer fault、tamper 与真实 cleanup uncertainty 均保持 fail-closed；terminal-proven 后只重试引用释放 bookkeeping，ticket 不提供任何 business transition，且不重放 outcome 不确定的 helper action；
3. **下一步**补贯穿 helper 阻塞点的 monotonic deadline/cancellation；随后实现 W09-B2b native strict-spawn production resolver、真实 process/pipe/select/kill/reap/FD liveness、numeric single-connect 与 peer/rebinding proof，再完成 B3 exact Transport 与 B4 全资源矩阵；

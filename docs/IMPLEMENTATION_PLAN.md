...
> **状态**：Active
>
> **实施基线**：`2026-09-01`，W09-B2a RESULT completion attestation 已提交并推送至 `main@d455c8e`；当前 factory-issued resolver lifecycle capability、spawn-before-return publication 与 cleanup-only recovery 封口仍在工作区，尚未提交
>
> **实现快照**：M0–M4、M5/W08、W09-A、W09-B0 与 W09-B1 已完成；W09-B2a 整体仍为 `in_progress`。远端 `main@d455c8e` 已包含 caller-owned `io_claimed`、terminal-guard/DNS START proof、冻结地址策略、injected helper lifecycle、唯一 offline coordinator、ledger-issued RESULT receipt/`ResolutionSet`、return-publication recovery，以及只在 stdout EOF、无第二帧、plain child exit status 0、single reap 与 pipe closure 全部证明后发行 receipt v2 的 completion attestation。当前未提交工作区再把 READY publication、lifecycle、transport claim 与 DNS START 四个 role ID 收入 launcher factory-issued、ledger-snapshotted exact capability，要求 spawner 在返回前把 kernel 发布进 ledger，并用 launcher 强引用 registry 保留 cleanup-failed owner；credential/attempt/Prepared cleanup 只消费独立 ledger snapshot，normal-return alias、public proof tamper 或 terminal callback fault 不能越权 START、错误终结其他 owner，终态会释放 context recovery refs。kernel attach、READY consume、credential claim/confirm、budget reserve、attempt claim、DNS START 与 credential borrow marker 的 transition 即使正常返回，也必须由独立 ledger observer 证明已提交；normal no-op 只能回滚到可重试状态或进入独立 cleanup-only state recovery，不能读取 secret、暴露 borrow view 或假报 terminal。helper resource action 完成而 `finish_cleanup` bookkeeping no-op 时只重试 bookkeeping，不重复 terminate/reap/close，launcher strong registry 也只能在 exact terminal 后释放。公开 coordinator 不再接受 caller proof UUID，launcher/guard 的 public lifecycle bypass 已封口，同 capability 并发只能 spawn 一次；production helper 仍零进程 fail-closed。B2a 还缺 deadline/cancellation 与对调用方可达的 cleanup-failure recovery ticket/retry；B2b native process/DNS/numeric connect/peer、B3 exact TLS/HTTP 与 B4 全资源矩阵也未实现，因此仍没有可发送 Transport。用户已有的 `README.md` 改动保持未暂存且未被本轮修改。应用仍被有意冻结，没有真实截图、真实人机预览、出站传输或可执行解题链。
>
...
| M5 | in_progress | Egress、session、Transport、预算、取消、授权租约、延迟密钥与统一清理 | W08、W09-A、W09-B0、W09-B1 与 B2a completion attestation 已推送；factory lifecycle/spawn publication/cleanup-only recovery 当前未提交，cancellation、external retry ticket 与 B2b–B4 DNS/peer/exact HTTP/TLS/全资源 cleanup 尚未实现 |
...
因此 M5 仍为 `in_progress`：W09-B2a 的 owner-bound AttemptGate、纯地址策略、injected helper lifecycle、唯一 offline coordinator、helper/RESULT wire v2、ledger-issued receipt/`ResolutionSet`、credential/READY return-publication recovery 与 completion attestation 已进入远端 `main@d455c8e`。当前未提交工作区把 READY publication、lifecycle、transport claim 与 DNS START proof 一次性由 launcher factory 生成并以 capability digest、exact ticket identity 和 ledger-owned snapshot 绑定；spawn kernel 必须先发布进 lifecycle ledger 才能从 spawner 返回，launcher 强引用 recovery registry 保留 cleanup-failed ledger/kernel。reservation、credential handle、attempt permit、terminal guard 与 Prepared cleanup 的正常返回别名/篡改窗口只走 exact owner 的 cleanup-only recovery；所有安全敏感 transition 的 normal return 还需独立 ledger postcondition，no-op 会被拒绝并回滚或经另一条 state recovery 精确终结，borrow owner 未提交时绝不开放 secret view；终态同时清掉 Attempt/Credential state 的 context/recovery refs。helper 的 `finish_cleanup` 只负责终态 bookkeeping：若它 no-op，已证明的外部资源动作不重放，ledger 保持可重试，strong registry 也保持到 exact terminal。公开 coordinator 不再接受三个 proof UUID，旧 public launcher/transfer/start/result-read 入口不存在，私有入口缺少 coordinator authority 时在任何外部动作前拒绝，reservation 状态原子执行 `reserved → launching → published → consumed`，因此同 capability 并发只能 spawn 一次。W09-B1 complete 只表示延迟 secret resolve、handle proof、owner-only one-shot borrow 与凭据终态合同成立；当前 B2a 仍只表示 fake-kernel 下的组合顺序、proof binding、返回值清理 ownership、completion attestation 与项目内防误用 capability 边界成立。B2a 还缺贯穿 helper 的 deadline/cancellation 与对调用方可达的 cleanup-failure recovery ticket/retry；W09-B 另缺 cancellable production DNS/native strict-spawn、numeric connect、TLS/hostname、精确单请求 HTTP/1.1 `send_once`、真实连接 peer/rebinding、redirect 禁止、bounded response 与完整 I/O cleanup。Phase 1 Transport 固定走自有的 exact 单请求 HTTP/1.1 路径，不使用 SDK/httpx 的隐式行为；禁止自动 retry、redirect、连接池复用、HTTP/2、proxy、chunked request 与透明压缩，任何额外 HTTP 请求都必须是新的获批 attempt。
...
当前未提交 capability/recovery-sealing 增量由 launcher 在 spawn 前生成四个互异 role ID，并将 spawn request digest 与它们 domain-separate 绑定为 immutable ticket；launcher reservation 与 lifecycle ledger 都保存独立 snapshot，READY guard 与 terminal guard 只持有同一 exact capability object。spawner 必须通过 publication sink 先把 kernel 锚定到 ledger；launcher registry 从该时刻强持有 ledger，直到 terminal proof 后才释放，terminal callback 的 bookkeeping fault 可由 cleanup recovery 幂等补做。cross-launcher、tamper、replay、旧 public API、无 authority 与 caller proof 参数均在外部动作前拒绝；同 ticket 并发只允许一次 spawn；reservation/READY/credential/attempt/Prepared 的 raise-after-return、normal-return alias 与 public proof tamper 只按独立 snapshot 回收 exact owner，合法的其他 ticket/handle/attempt/guard alias 保持不受影响。kernel attach、READY acceptance、credential claim/confirm、budget reservation、attempt claim、DNS START 与 borrow begin/finish 均有 normal-return commit postcondition；wrapper recovery 的返回布尔值不构成 terminal 证据，必须由 cleanup-only observer 复核，必要时走独立 state path。helper 外部动作证明完成而 `finish_cleanup` no-op 时，状态保持 `cleaning` 供 bookkeeping-only retry，绝不重复 terminate/reap/close；真实 action proof 不确定则保持 `cleanup_failed` 与 strong recovery anchor。终态同步释放 Attempt/Credential recovery refs。完整离线 suite 为 532/532，W09 为 227/227，核心五模块为 172/172；101 个 Python 文件通过 3.10 AST grammar，`compileall`、依赖方向与 `git diff --check` 通过，本机仍为 Python 3.12.13。B2a 下一步补贯穿 helper 的 monotonic deadline/cancellation，以及 cleanup fault 后对调用方可达且可重试的 recovery ticket；当前 `cleanup_failed` 只保留内部 strong recovery anchor，尚无 external retry。B2b 再补 native helper/spawn shim、真实 process/pipe/select/kill/reap/FD liveness、DNS、numeric socket/connect/getpeername；B3 exact HTTP/TLS 与 B4 全资源矩阵仍待完成。当前不包含真实进程、DNS/socket/HTTP 或 Provider API，因此 W09-B2/M5 继续保持未完成。
...
| W09-B2a | owner、地址策略与 helper 离线生命周期 | W09-B1 | `runtime/attempt.py`、`transport/address_policy.py`、`transport/resolver.py`、`transport/http.py` | `io_claimed` owner/guard、READY/transfer/single START、16 KiB/32 条整组地址策略、唯一 offline coordinator、ledger-issued RESULT/Resolution binding、factory capability/kernel prepublication/EOF-exit-reap attestation/cleanup-only recovery、production fail-closed；deadline/cancel 留下一切片 |
...
当前工作包状态：W01–W09-B1 complete，W09-B2a completion attestation 已推送至 `main@d455c8e`；整个 W09-B2a 仍为 `in_progress`，factory lifecycle capability、kernel prepublication、strong recovery anchor、cleanup-only snapshots 与 public bypass 封口当前尚未提交，W09-B2b–B4 与 W10–W15 为 `pending`。当前增量只表示 fake source/kernel 下的 factory role proof、single-spawn、exact-owner recovery、EOF/无第二帧/exit-zero/single-reap/pipe-close 发行门槛存在，仍不表示 production credential source、deadline/cancellation、external cleanup retry ticket、resolver/process、DNS/numeric connect/HTTP Transport、Provider、真实 macOS 捕获、传输或发布链可用，因此 W09-B2 与 M5 保持 `in_progress`。
...
1. **已完成并推送** M5/W08、W09-A、W09-B0、W09-B1，以及 W09-B2a 的 owner/address/helper、唯一 offline coordinator、RESULT receipt/`ResolutionSet` ledger binding、return-publication recovery 与 completion attestation，远端基线为 `main@d455c8e`；
2. **当前工作区已实现、尚未提交** W09-B2a capability/recovery sealing：launcher 在 spawn 前生成并 snapshot 四个 role proof，冻结 request/spawner 构造绑定，kernel 在 spawner 返回前发布进 ledger，launch owner、strong registry 与 cleanup-only snapshots 覆盖 alias/tamper/callback fault；公开 coordinator 不再接收 caller proof UUID，旧 public lifecycle bypass 已封口，同 ticket 并发恰好一次 spawn；normal-return postcondition、独立终态观察与 state recovery 已覆盖 credential/attempt/helper 的关键 transition；离线 suite 532/532、W09 227/227、核心五模块 172/172；
3. **下一步**补贯穿 helper 的 deadline/cancellation 与对调用方可达的 cleanup-failure recovery ticket/retry；随后实现 W09-B2b native strict-spawn production resolver、真实 process/pipe/select/kill/reap/FD liveness、numeric single-connect 与 peer/rebinding proof，再完成 B3 exact Transport 与 B4 全资源矩阵；
...
- 当前未提交 capability/recovery-sealing 增量：launcher factory 在 spawn 前生成 READY publication、lifecycle、transport claim 与 DNS START 四个互异 role ID，以 capability digest、reservation snapshot、exact ticket identity、构造时 request/spawner snapshot 与 lifecycle ledger snapshot 绑定；kernel 必须由 publication sink 在 spawner 返回前锚定，launch owner 隔离并发赢家/输家，launcher strong registry 在 terminal 前保留 recovery owner。公开 coordinator 不再接收 caller proof UUID，旧 public launcher/guard lifecycle API 已移除，私有入口缺少 authority 时零外部动作；cross-launcher/replay/tamper 在 spawn 前拒绝，同 ticket 并发只有一个赢家。normal-return alias、return-then-raise、public proof tamper、cleanup action/callback fault 的 cleanup-only recovery、独立 commit/terminal observer、state recovery 与 terminal ref release 已加入回归。当前完整离线测试 532/532、W09 227/227、核心五模块 172/172；101 个 Python 文件已通过 3.10 AST/compileall 复核。deadline/cancellation、external cleanup-retry ticket、B2b native process/DNS/numeric connect/peer、B3 exact HTTP/TLS 与 B4 全资源矩阵仍待完成，本增量尚未提交。

...
> **状态**：v3 实现基准。本文描述目标架构；远端 `main@62c53c1` 已完成并推送 M0–M4、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 第一批离线安全原语，包括 owner-bound attempt、纯地址策略和可注入 helper 生命周期。当前未提交工作区继续推进唯一 offline coordinator 与 proof-bound RESULT/`ResolutionSet` v2；W09-B2a 与 M5 仍为 `in_progress`，这仍不代表 v3 用户链路已经可用。交付顺序、状态与逐项验收见 [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)。
>
> **已定方向**：模型能力分为两条推理通道：
>
...
- **M5/W09-B2a in progress（第一批已推送至 `main@62c53c1`，后续当前未提交）**：已推送的第一批把 AttemptGate 的无主 claim 收紧为 caller-owned `active → claiming → io_claimed`，加入 exact terminal-guard 与 one-shot DNS START authority proof，并冻结 IANA-2025-10-09 地址策略、16 KiB/32 条整组拒绝、确定性去重排序、AttemptPermit-bound `ResolutionSet`、pure peer matcher 及 injected helper READY/transfer/single-START 生命周期。当前未提交增量新增唯一 offline coordinator，按 READY → credential resolve → attempt reserve/claim → helper transfer/guard bind → DNS START commit → single START/RESULT 顺序组合这些原语；helper wire 已升级为 v2，factory-only RESULT receipt 与 `ResolutionSet` v2 由 helper ledger 独立快照 exact 绑定 claim、terminal guard、DNS START、START frame、canonical target/policy 和原始 transcript/candidates。credential 与 READY 的 return-publication cleanup 分别使用 per-call ID 和 caller-owned reservation/ticket，不能跨调用回收 owner。production spawner 仍零进程 fail-closed；完整离线测试 466/466。B2a 仍缺 factory-generated READY/lifecycle proof 与 public bypass 封口、deadline/cancellation、child EOF/exit/reap 成功证明及 cleanup-failure recovery ticket/retry，因此尚未 complete；
...
远端 `main@62c53c1` 已包含 W09-B2a 第一批离线原语：AttemptGate 的 transport claim 由 caller UUID 独占并线性化为 `io_claimed`，helper handoff proof 与 DNS START proof 均只能使用一次；`ResolutionSet` 从 AttemptPermit 私有冻结绑定派生 host/port/policy，并对 canonical transcript 执行上述整组地址策略；helper lifecycle 仅通过 injected fake kernel 验证 READY、owner CAS、single START、bounded result 与 cleanup 状态。当前未提交增量再加入唯一 offline coordinator，固定 READY → credential resolve → attempt reserve/claim → helper transfer/guard bind → DNS START commit → single START/RESULT 的组合顺序；helper protocol/READY/START/raw RESULT 升级为 v2，terminal guard 发行 factory-only `ResolverResultReceipt`，ledger 独立保存 exact START frame 与 receipt/transcript 快照，并把单次 `ResolutionSet` 及每个 candidate 的 identity/digest/canonical payload 锚定到同一发行事实。active/unclaimed attempt、跨 START/attempt/receipt replay、terminal receipt、对象内摘要重算或任一 echo 不一致均不能发布或选择地址；credential resolve 与 READY return-then-raise 的 cleanup recovery 还分别绑定本次 publication ID 与 spawn 前预占的 exact ticket。后者只解决返回值丢失窗口，不等于 production lifecycle capability：caller 仍可直接取得 public launcher/guard primitive，READY/lifecycle/owner proof 仍由 caller 提供，尚无 factory-only capability 封口；也没有贯穿 helper 的 monotonic deadline/cancellation，当前首个 RESULT frame 后还未证明 stdout EOF、无第二帧、child exit status 0 与成功路径 reap，cleanup fault 后也没有可达的 recovery ticket/retry；native kernel/helper executable/spawn shim、真实 process/pipe/select/kill/reap/FD liveness、实际 DNS、numeric socket/connect 或 `getpeername()` I/O 同样不存在。`FailClosedProductionHelperSpawner` 在这些证据存在前始终零进程 fail-closed，因此 W09-B2a、W09-B2 与 M5 均不得标记 complete。
...
**当前实现状态（2026-08-31）**：M0–M4、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 第一批离线安全原语已进入远端 `main@62c53c1`；M5 与整个 W09-B2a 仍在进行。当前未提交工作区新增唯一 offline coordinator、helper/RESULT wire v2、ledger-issued RESULT receipt、单次 ledger-anchored `ResolutionSet`，以及 credential/READY 的 per-call return-publication recovery；proof exact 绑定 claim、terminal guard、DNS START/START frame、canonical target/policy、原始 transcript 与 candidates。完整离线测试 466/466，W09 为 161/161，当前共 102 个 Python 文件；本机实际运行时仍为 Python 3.12.13，真实 Python 3.10 runtime suite 证据仍缺失。这些路径只使用 fake source/kernel，保持零真实环境凭据、零真实进程、零 client、零 DNS/socket/HTTP。production credential source、factory-generated READY/lifecycle proof 与 public bypass 封口、deadline/cancellation、RESULT 后 EOF/无第二帧/child exit-zero/reap success attestation、cleanup-failure recovery ticket/retry、cancellable native strict-spawn helper、真实 process/DNS/numeric connect/peer、B3 exact HTTP/TLS 及 B4 全资源矩阵仍不存在，因此 W09-B2a/W09-B2/M5 不得升级为 complete。GLM 仍没有 VerificationRecord，只派生 `experimental`；应用仍被有意冻结，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、Provider API、live smoke 或真实 macOS E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

`M1–M4`、M5/W08、W09-A、W09-B0 与 W09-B1 已经完成；W09-B2a 第一批离线原语已推送，唯一 offline coordinator、RESULT receipt/`ResolutionSet` ledger binding 与 return-publication recovery 已在当前未提交工作区实现。下一步先补 RESULT 后 EOF/无第二帧/child exit-zero/reap success attestation，再用 factory-generated READY/lifecycle capability 封住 public bypass，并补齐贯穿 helper 的 deadline/cancellation 与 cleanup-failure recovery ticket/retry；随后完成 B2b native production resolver、numeric single-connect 与真实 peer/rebinding proof，再依次完成 B3 exact Transport 与 B4 完整矩阵。首次真实 API 只能发生在 M6/W11，且需要另行明确授权、只用固定非敏感合成图、只调用一次并关闭自动 retry；真实用户截图只能在该 smoke 完成后的 M7/W12 接入。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。

...
> **状态**：v3 实现基准。本文描述目标架构；远端 `main@5468b52` 已完成并推送 M0–M4、M5/W08、W09-A、W09-B0、W09-B1，以及 W09-B2a 的 owner/address/helper、唯一 offline coordinator、proof-bound RESULT receipt/`ResolutionSet`、completion attestation 与 capability/recovery sealing 离线增量。当前未提交工作区继续推进 caller-owned `ResolverCleanupTicket` 聚合恢复；W09-B2a 与 M5 仍为 `in_progress`，这仍不代表 v3 用户链路已经可用。交付顺序、状态与逐项验收见 [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)。
>
> **已定方向**：模型能力分为两条推理通道：
>
...
**当前实现状态（2026-09-02）**：M0–M4、M5/W08、W09-A、W09-B0 与 W09-B1 已完成；W09-B2a 的 owner/address/helper、唯一 offline coordinator、RESULT receipt/`ResolutionSet` binding、completion attestation 与 capability/recovery sealing 已推送至远端 `main@5468b52`，M5 与整个 W09-B2a 仍为 `in_progress`。当前未提交工作区新增 caller-owned `ResolverCleanupTicket` 与三层 aggregate recovery ledger：factory attach、bind 及每个 helper/credential/attempt/guard 风险边界都有独立 normal-return postcondition；cleanup 固定按 helper → Attempt/Gate → credential 推进，只信 ledger terminal observer，不信 recovery 返回值。public retry 在 issued/coordinating 时不取消，在瞬时 wrapper fault 后恢复 recoverable；`cleanup_failed` 不重放 outcome 不确定的 helper action。三层资源证明完成后先冻结 `resources_terminal_proven`，引用释放或其 observer 失败只重试 bookkeeping；terminal credential state 只保留 primitive owner proof。`publish_recoverable` 是 return 前的 cleanup ownership 线性化点，并发 ticket cleanup 可以使返回的 Prepared 已同步为 closed。当前完整离线 suite 为 557/557，W09 为 252/252，核心五模块为 197/197，coordinator 为 82/82；101 个 Python 文件通过 3.10 AST grammar、`compileall`、依赖方向与 `git diff --check`。本机实际运行时仍为 Python 3.12.13，真实 Python 3.10 runtime suite 证据仍缺失。这些路径只使用 fake source/kernel，保持零真实环境凭据、零真实进程、零 client、零 DNS/socket/HTTP。production credential source、helper deadline/cancellation、cancellable native strict-spawn helper、真实 process/DNS/numeric connect/peer、B3 exact HTTP/TLS 及 B4 全资源矩阵仍不存在，因此 W09-B2a/W09-B2/M5 不得升级为 complete。GLM 仍没有 VerificationRecord，只派生 `experimental`；应用仍被有意冻结，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、Provider API、live smoke 或真实 macOS E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

`M1–M4`、M5/W08、W09-A、W09-B0 与 W09-B1 已经完成；W09-B2a capability/recovery sealing 已推送至 `main@5468b52`，caller-owned `ResolverCleanupTicket` aggregate recovery 当前已实现但尚未提交。下一步仅补齐贯穿 helper 阻塞点的 monotonic deadline/cancellation；随后完成 B2b native production resolver、numeric single-connect 与真实 peer/rebinding proof，再依次完成 B3 exact Transport 与 B4 完整矩阵。首次真实 API 只能发生在 M6/W11，且需要另行明确授权、只用固定非敏感合成图、只调用一次并关闭自动 retry；真实用户截图只能在该 smoke 完成后的 M7/W12 接入。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。

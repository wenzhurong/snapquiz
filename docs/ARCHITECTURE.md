...
- **M5/W09-B1 complete（`main@543d390`，已推送）**：已新增 factory-only `CredentialHandle`、私有 mutable-secret ledger、built-in GLM exact frozen-binding resolver、caller handle proof、AttemptPermit v2 与 owner-only one-shot borrow。exact-one-read、post-read authority、Bearer 边界、handle 返回前预占、resolver/close/borrow 的单故障与 commit-then-raise、并发 owner/close/finish、source traceback 与零 I/O 均有回归；完整离线测试 401/401，独立终审未发现剩余 P0/P1；
- **W09-B2a complete（M5 子项）**：`62c53c1` 的第一批把 AttemptGate claim 收紧为 caller-owned `active → claiming → io_claimed`，加入 exact terminal-guard/DNS START proof、冻结地址策略与 injected helper 生命周期；`1ac6fca` 的第二批又加入唯一 offline coordinator、helper wire v2、factory-only RESULT receipt、单次 ledger-anchored `ResolutionSet`，以及 credential/READY return-publication recovery；`d455c8e` 再把 stdout EOF、无第二帧、plain exit status 0、single reap 与 parent pipe closure 设为 receipt 的联合发行门槛；`5468b52` 已由 launcher factory 生成并 snapshot 四个 role proof，冻结构造时 request/spawner，移除 caller UUID 与 public lifecycle bypass，要求 kernel 在 spawner 返回前进入 ledger，并以 launch owner、strong registry、cleanup-only snapshots、独立 commit/terminal observer 与 state recovery 覆盖 alias/tamper/callback/no-op fault；`8047b9d` 再提供调用前预持、三层聚合且 cleanup-only 的 `ResolverCleanupTicket`，异常后不会丢失恢复入口，也不会把 action uncertainty 假报为 terminal；`5fe5eab` 完成 Gate-issued exact `HelperStopAuthority`、50 ms helper poll、stale/no-op `HelperWaitSlice` postcondition、direct `ResolutionSet` publication gate、每动作 8×50 ms cleanup 与独立 helper-stop release proof。B2a 的最终证据为完整离线 580/580、W09 九模块 275/275、核心五模块 198/198（attempt 36/36、context 12/12、address 17/17、lifecycle 43/43、coordinator 90/90），101 个 Python 文件通过 3.10 AST grammar、`compileall` 与 `git diff --check`；独立终审无剩余 P0/P1；
- **W09-B2b 本地可完成范围 complete，production blocked（`main@341374b`，已推送）**：既有 native process/message-IPC、S1 pure/local supervisor、S2a fixture bootstrap 与 S2b-I1 connection-peer identity 之外，远端基线已加入 S2b-I2 suspended identity/process watcher、S2c pre-secret startup composition、S3 canonical wire + durable zero-child proxy、S4 async event owner/output cache/native resolver owner adapter、S5 strict local helper/DNS，以及 S6 numeric/TLS native owner、C-to-C transfer 与 exact-peer foundation。它们分别冻结本地合同与失败边界，但 production startup、真实系统 vtable、final signed artifacts 和 app integration flags 都保持 false；
- **W09-B3/B4 本地可完成范围 complete，production blocked（`main@341374b`，已推送）**：`_exact_tls.py`、`_exact_http1.py` 与 `_exact_transport.py` 已组成 exact TLS/HTTP/1.1、zero redirect/retry 的 fake-edge single-request Transport；Keychain bridge 与 native build 分别提供独立 source/build evidence，readiness v5 则冻结 capability interface、app/entitlement 与 cutover 的 exact exit shape；B4 已覆盖 fault/race/cleanup 与 fail-closed production exit inventory。三轮独立退出审查最终 P0/P1=0；Python 3.12.13 与 Python 3.10.20 完整离线 suite 均为 1235/1235，但这些证据不构成 production completion；
- **尚不可用**：fixed Team-signed app/supervisor/helper/native artifacts、最终 bundle/signing/Keychain entitlement facts、真实 process/DNS/socket/SSL/Security.framework vtable 与 Keychain item、S7/app cutover、真实 macOS resolver/transport liveness、Quartz/TCC、真实 display/选区/截图 source、NSPanel 真人预览、live/eval/E2E 仍未完成或未获授权。S7 未启用，所有 production availability/readiness flags 保持 false，所以当前没有可执行的解题用户路径；Registry 中的 `experimental` 只是 exact binding 状态，不代表应用链已可运行或已经 `supported`；
- legacy `Config`、parse/notify 与 `AnswerResult` 仍留在源码树中，但被 M0 产品入口隔离；旧布尔权限 helper 已永久 fail-closed，新 W06–W09 local/offline foundation 链尚未接入应用。默认产品链仍不截图、不读取真实 credential、不构造 SDK 或联网；`SendSessionFactory` 只消费 approval/签发 session，仍不解析 credential。one-shot Consent 也仅能由 exact session-bound `ConsentUseLease` 延续，缺失/失配时继续 fail-closed。
...
7. CaptureArtifactFactory 返回 Provider-neutral `CaptureArtifact`，不生成 OpenAI 专用 data URL。W06 只接受 canonical PNG；InputValidator 在捕获后再次复核 privacy/permission/topology/Plan，并真解码图片、核对 magic/CRC/zlib/真实尺寸/像素/字节/空白/黑帧，成功后只签发绑定完整 artifact metadata 的 `ValidatedCapture` lease。W10 进一步要求原 capture ledger 在 validator 返回后以 exact 对象 identity 与 validation digest 独立证明 publication；return-loss 只可由同一 ledger 恢复并释放，foreign return/recovery alias 不得进入预览、Transport 或由当前执行器释放。JPEG 支持必须先加入同等严格的 decoder/policy/version，不得因为领域枚举允许 JPEG 就自动放行。
...
9. EgressGate 复核选区、AuthorizationContext、plan snapshot、exact endpoint/data-kind，以及最终 payload 的 digest/字节数；当前 pass-through policy 必须在生成预览前用可信 Adapter 独立重建一次并逐字段、逐字节比对 `PreparedOutbound`，从而证明预览图片就是 body 内嵌的 canonical PNG。受控 PreviewController 返回后，Gate 只重算完整 review subject 与 exact bytes，不第三次调用 Adapter；明确批准后才为该 network operation 产生限时、单次 `EgressApproval`。W08 只建立 UI boundary 合同，真实 NSPanel 人机证据属于 W12。Gate 失败、取消或 payload 不匹配时，密钥读取、DNS、socket 与 HTTP 调用次数都必须为零，并立即释放临时 artifact/payload。
...
W10 将上述步骤组合为一个 local/offline reference executor，并进一步冻结以下跨层合同：

- 凡是发布可变 authority 或资源 owner 的 factory（包括 `RuntimeCallFactory`、`InputValidator` 的 sensitive lease publication、`AttemptGate` 与 resolver coordinator），其正常返回都不是 authority 事实来源。caller 必须在下一风险动作前用账本私有 observer 证明 exact publication；纯值 factory 仍由不可变类型、digest 与完整性合同验证。authority observer 只能接受严格 `bool is True`，truthy、normal-return no-op、partial write、commit-then-raise 与另一个合法对象的 return alias 均不得前进；
- executor 必须在 context start、capture validation、credential permit 与 resolver coordination 的风险窗口保留对应 cleanup owner。Context 未返回时的 absence proof 必须限定于本次预持的 `start_owner`；同一 `request_id` 的合法 foreign Context/pending start 既不能被当前执行采纳或关闭，也不能因为全局 key 仍存在而永久阻塞本次 cleanup，畸形或 owner 不一致状态继续 fail-closed。CapturePolicy 的 authorization/consumption 返回必须先由原 ledger 证明 exact identity/digest，validation recovery 所需的 ledger+consumption 必须以一个 holder 原子发布，不能因两次状态写入之间的异步中断形成永久 pending。`ValidatedCapture` 返回后、任何 preview/secret/Transport 前，必须由原 capture ledger 证明 exact identity/digest publication；返回丢失由原 ledger 恢复释放，foreign lease 保持归属不变。承载该 lease 的 `SolveRequest` 与 `StageInvocation` 返回还必须复核 exact input identity 与完整 digest 链。`PreparedResolverAttempt` 返回后、Transport 调用前，还须证明它由当前 caller 的 exact `ResolverCleanupTicket` 锚定，并同时绑定当前 launcher、credential resolver、AttemptGate、CredentialResolutionPermit、context/session/operation 与 request envelope。foreign Prepared 不得被发送、采纳或由当前 executor 关闭；本次 coordinator 实际发布但返回丢失的 owner 则只由本次 ticket cleanup；observer 通过后若 ticket cleanup 先于 transport claim 获胜，Transport 必须在任何 socket/TLS 动作前拒绝，反向顺序则由 claim 排除 cleanup 直至 exact finish；
- capture consume 与真实捕获之间必须由 `CallContextLedger` 原子提交 capture-start claim；result validation 与对调用方交付之间必须由同一 Context 原子提交 result-publication claim。取消、deadline、Registry/Policy revoke 与这两个 claim 只有一个线性化赢家：claim 前获胜就阻止对应副作用，claim 后获胜不能倒推“未发生”；
- 一个 `MultimodalCancellation` 只绑定一个 live execution owner。取消 intent 必须先锁存，context cancel 的 precommit、normal-noop 或 commit-then-raise 都由后续 checkpoint 重驱和独立观察；另一个 executor 的失败/cleanup 不得释放当前 owner 的 binding；
- executor 是 single-flight。active marker 与 recovery state 必须先后精确发布、精确移除；任何 bookkeeping outcome 不确定都保留 cleanup-only recovery，不能丢失已确定结果或重放业务动作。`retry_cleanup_and_finalize()` 只允许继续 terminalization，并交付最初冻结的 typed error 或已 claim 结果；严禁重新截图、重新读取 secret、重新解析、重新发送或重新 decode；
- Adapter 选择必须通过 `(adapter_family, adapter_version, adapter_type)` 的受信目录解析，核心 Egress 逻辑不得出现“只有某个 Provider 类才合法”的条件分支。每个具体 Adapter 仍须自行 exact 验证其 profile/protocol；新增目录项不代表 supported，也不绕过 Egress rebuild、live/eval/E2E 门槛；
- 当前 Egress 的 prepare 次数严格为两次：caller Adapter 生成待审查 envelope 一次，Egress 从受信目录独立重建一次；preview 后只重算完整 authority subject 与 preview，不得让 Session、Attempt、resolver 或 Transport 再 prepare。ApprovalLedger 仅保留 digest attestation 与 exact ledger identity；不可证明的 partial lifecycle transition 必须 rollback 到 exact 原状态，否则 ledger poison，不能返回可复用 approval/session；
- 最终 ResultValidator 的正常返回只是一项纯值计算结果，executor 还必须独立复核 candidate 的完整 correlation/integrity，并以相同 candidate payload 与本地 provenance 重建期望结果；只有字段逐项等价且 provenance 是当前 exact 对象时才可进入 result-publication claim，合法但不同答案的 return alias 必须拒绝；
- W10 production factory 必须保持 unavailable，直到 W09 production exit gates 被外部证据关闭并完成受审查接线。W10 的合成 PNG、fake credential/resolver/kernel/numeric/TLS edge 只能证明组合、故障与清理，不证明真实 TCC、屏幕捕获、Keychain、DNS/socket/TLS/HTTP、Provider 可用性或模型质量。
...
PayloadPreparer 必须是确定性的纯本地步骤。`request_envelope_digest` 必须覆盖 method、完整规范化 URL（含非敏感 query）、content type、应用控制的非敏感 headers、credential-binding digest（无认证时为明确的 `not_applicable` 标记）与 body digest。W08 的 EgressGate 每次只审批一个 PreparedOutbound，并重新证明 AuthorizationContext/grants 未过期、未撤销且与 plan digest 匹配；当前 phase1 pass-through policy 在生成预览前调用一次可信、Registry-bound Adapter 的 `prepare()`，逐项比较所有 PreparedOutbound 字段与 exact body bytes，禁止调用方用另一个自洽 envelope 冒充 Adapter 产物；预览返回后只重算完整 subject 与 exact bytes，不再次 prepare。未来 Adapter 必须接入等价的可信 preparer dispatch，不能删除该 provenance 证明。
...
当前 `_darwin_keychain_source.py` 提供 production-unwired 的 Keychain staged-source foundation：调用方必须先预持固定 4096-byte mutable buffer，source exact 绑定 frozen service/account/access-group mapping 与 binding digest；native read、publication receipt、commit 和 CredentialResolver bridge 各自有独立 ledger postcondition，detach→commit、reentrant read、receipt TOCTOU、异常与回收路径均不得暴露或遗留 secret。成功后只允许 readonly one-shot borrow，失败与终态均清零 caller buffer。该 foundation 使用 injected Security.framework backend，既不读取真实 Keychain item，也不证明应用的 access-group entitlement；production 必须把真实 `SecItemCopyMatching` 等受审查 vtable 与最终 Team-signed app/entitlements exact 绑定后再取得运行证据。
...
提交 `d455c8e` 及之前的 W09-B2a owner/address/coordinator/result 离线增量规定：AttemptGate 的 transport claim 由 caller UUID 独占并线性化为 `io_claimed`，helper handoff proof 与 DNS START proof 均只能使用一次；`ResolutionSet` 从 AttemptPermit 私有冻结绑定派生 host/port/policy，并对 canonical transcript 执行整组地址策略；唯一 offline coordinator 固定 READY → credential resolve → attempt reserve/claim → helper transfer/guard bind → DNS START commit → single START/RESULT 的组合顺序。helper protocol/READY/START/raw RESULT 为 v2；terminal guard 发行 factory-only `ResolverResultReceipt`，ledger 独立保存 exact START frame 与 receipt/transcript/completion 快照，并把单次 `ResolutionSet` 及每个 candidate 的 identity/digest/canonical payload 锚定到同一发行事实。active/unclaimed attempt、跨 START/attempt/receipt replay、terminal receipt、对象内摘要重算或任一 echo 不一致均不能发布或选择地址；credential resolve 与 READY return-then-raise 的 cleanup recovery 分别绑定本次 publication ID 与 spawn 前预占的 exact READY lifecycle publication capability（非 `ResolverCleanupTicket`）。

提交 `d455c8e` 还把首个 RESULT frame 后的单字节 EOF probe、无第二帧、child plain exit status exact 0、成功 single reap 与 parent-side pipe closure 全部纳入 receipt v2 payload、digest 和 ledger-owned snapshot；receipt 只能在上述证明全部提交后发行。reap/close 在外部调用前由 ledger claim，异常或 return-then-raise 后不允许第二次动作；一旦 reap 已 claim，也不再 terminate 可能已回收并复用的进程身份。合法路径在返回 `PreparedResolverAttempt` 前已经 reap child 并关闭 pipes，后续 `close()` 只终结 helper ledger、Gate attempt 与 credential handle。非零 plain status 可证明 child 已回收但禁止发行；类型非法或 reap/close 结果不确定会保留 `cleanup_failed` recovery anchor。

已推送的 `main@5468b52` capability/recovery-sealing 增量规定：`ResolverHelperLauncher` 必须在任何 spawn 前一次性生成互异的 READY publication、lifecycle、transport claim 与 DNS START role ID，并用 spawn request digest 生成 domain-separated capability digest；reservation state 与 lifecycle ledger 各自保存独立 snapshot，READY/terminal guards 只能绑定同一 exact lifecycle capability object。spawner 只有先通过单次 publication sink 把 kernel 锚定到 lifecycle ledger 才能正常返回；launcher 从 spawn 前开始以 strong recovery registry 保留 ledger/kernel，只有 exact terminal resource proof 后才释放，terminal callback 的 registry fault 必须能由 cleanup recovery 幂等补做。公开 `coordinate_resolver_attempt` 不接收这些 proof UUID；launcher、ownership transfer、START 与 RESULT read 只保留要求 module-private coordinator authority 的私有入口，旧 public lifecycle primitive 不存在。reservation 必须原子执行 `reserved → launching → published → consumed`，所以同 lifecycle capability 并发只能一次 spawn；cross-launcher、tamper、replay、无 authority 或 caller proof 参数都必须在任何外部动作前失败。credential/attempt/Prepared cleanup 只能使用 ledger/factory 独立保存的 permit、handle、guard 与 publication snapshot；raise-after-return、normal-return alias 或返回对象字段篡改都不能授权 START、回收另一个 owner，或把完整性失败误判成 terminal，合法的其他 owner alias 也不得受影响。kernel attach、READY consume、credential claim/confirm、budget reserve、attempt claim、DNS START 与 credential borrow begin/finish 在正常返回后都必须由独立 ledger observer 证明 exact commit；normal no-op 必须在 secret read/view、DNS START 或后续状态前失败，并回滚到可重试状态或走另一条 cleanup-only state recovery。wrapper recovery 的返回布尔值本身不是终态证据；只有 cleanup-only observer 证明 terminal 后才可清零 handle、释放 context refs 或 strong registry。若 helper 外部 terminate/reap/pipe-close proof 已完成而 `finish_cleanup` bookkeeping no-op，ledger 保持 `cleaning` 并只重试 bookkeeping，不得重复任何外部 action；action proof 不确定则保持 `cleanup_failed` 与 strong recovery anchor。Attempt 与 Credential state 一旦 terminal 必须同步清空 context/context-ledger/recovery refs。该边界用于项目内防误用，不宣称抵抗已取得任意同进程反射/私有 token 的恶意代码。

已推送的 `main@8047b9d` caller-owned recovery 增量再规定：调用方必须先以零 I/O `issue_resolver_cleanup_ticket()` 取得 factory-only、runtime-final、不可复制/序列化的 `ResolverCleanupTicket`，再把 exact ticket 作为 `coordinate_resolver_attempt(..., cleanup_ticket=...)` 的必填参数；factory attach 与 bind 的正常返回都必须经独立 postcondition。同 ticket 只能 bind 一次；coordination owner 必须在 bind 前创建，使 bind commit-then-raise 仍可精确恢复，而未完成 ownership transfer 的 bind no-op 不得取消 caller permit，解除故障后同 ticket 仍可使用。除 `policy_version` 与安全 `repr` 外，ticket 只公开 `safe_metadata()`、`is_terminal` 与 `retry_cleanup()`，不得暴露 credential handle/permit、helper guard、secret、START/RESULT、reserve/claim 或 borrow capability。私有聚合 ledger 在每个风险动作前冻结并独立确认 launcher/reservation/launch、credential publication/handle、Attempt 与 terminal guard snapshot，随后严格按 helper terminal → Attempt/Gate terminal → credential handle/Gate terminal 推进；credential ledger 的终态观察只保留 primitive owner proof，不在长寿命 state 中强持有 permit/Gate。recovery 方法返回 true 不构成完成证明，每层都必须由另一个只读 ledger observer 证明 exact terminal 后才可释放强引用。`publish_recoverable` 是 coordinator 返回 `PreparedResolverAttempt` 前的 cleanup ownership 线性化点，且正常返回还必须由 `prepared_publication_is_committed` 独立确认；该点后 caller ticket 可以并发赢得 cleanup，因此尚未完成 Python return 的 coordinator 仍可返回一个已同步为 closed 的 Prepared，未来任何 B2b consume 必须原子拒绝 terminal ledger。Prepared 与 ticket 共用同一聚合 ledger，所以 Prepared 正常返回后被外层丢失仍能由调用方清理；任一侧完成后另一侧幂等观察 terminal。ticket/ledger 不持有 primary 或 traceback；coordinator 以 bare raise 保留原异常的 identity、type 与 traceback，cleanup fault 不替换它，也不作为 cause/context 附着。

`retry_cleanup()` 在 issued 或 coordinating 状态只返回 false，不是 cancellation，也不能终止尚未到达 publication 线性化点的 coordinate；并发 retry 必须串行化，瞬时 cleanup wrapper fault 只使 public retry 返回 false 并把 ledger 恢复为 recoverable，terminal 后则幂等返回 true。retry 路径禁止调用 resolve、reserve、claim、DNS START、RESULT read 或 secret borrow。三层独立 observer 全部通过后，aggregate 先不可逆记录 `resources_terminal_proven`；若随后 strong-ref release 或其 observer 失败，ticket 仍保持 recoverable，后续 retry 只能补 ref-release/terminal bookkeeping，不能再运行 helper、Attempt 或 credential cleanup。只有该 proof 已记录且全部聚合资源引用被独立观察为释放，ticket 才可转为 terminal。helper external action 已证明完成、只剩 `finish_cleanup` bookkeeping no-op 时同样只补 bookkeeping，不得重复 terminate/reap/close；一旦 action outcome 不确定而进入 `cleanup_failed`，ticket 必须保持 recoverable/nonterminal，保留 Attempt 与 secret owner，即使故障随后移除也不得重放未知动作；此处 recoverable/retryable 只表示 cleanup-only owner 仍可安全调用，不保证状态能够推进，每次调用都可能继续返回 false。`main@8047b9d` 是尚未覆盖 helper monotonic deadline/cancellation 的历史基线；下述能力已由 injected-fake 收口并随 B2a 在 `main@5fe5eab` complete。native resolver/output、numeric/TLS、C-to-C transfer 与 Keychain owner foundations 随后已推送到 `main@341374b`，但它们仍是 injected-vtable、production-unwired 实现；fixed Team-signed artifacts、真实系统 vtable/entitlement 和真实 liveness/transport evidence 仍不存在。`FailClosedProductionHelperSpawner` 在这些证据存在前始终零进程 fail-closed，因此整个 W09-B2 与 M5 均不得宣称 production complete。

W09-B2a deadline/cancellation 离线收口实现如下 required 合同：`AttemptGate` 必须在任何 lifecycle reservation 或 spawn 前，从 exact `CredentialResolutionPermit` 经既定五账本 authority path 签发唯一、factory-only、runtime-final 的 `HelperStopAuthority`。该 authority 必须同时 exact 绑定 Gate/permit、`CallContext`/ledger、session/terms、`MonotonicDeadline` 与 `CancellationToken` 的对象身份、ID 和 digest，并由同一 helper lifecycle capability snapshot；caller 和公开 coordinator 均不得提交 raw `now`、deadline、duration、token、authority ID 或 bypass bool。`effective_deadline_ns` 必须取 request monotonic deadline 与 exact session `valid_until` 在 `CallContextLedger` 内保守映射所得 monotonic deadline 的较小值；每个 factory-only `HelperWaitSlice` 都必须通过完整 authority path 重新取得 trusted `ClockSample.monotonic_after_ns`，令 `max_wait_ns = min(remaining_ns, 50_000_000)`。每次 slice 正常返回后必须由 Gate 内部独立 ledger postcondition exact 确认本次 sequence/identity/digest/phase/deadline；前一 slice 立即 stale，不得 replay，callback normal-return no-op 或替换返回值都不能授权一次 poll。同一 checkpoint 同时观察到取消和到期时先返回 `CancelledError`；有效区间保持半开，`observed_monotonic_ns == effective_deadline_ns` 已是 `TimeoutError`。clock rollback、authority alias/tamper、cross-context/session/lifecycle 或 no-op publication 全部 fail-closed。

所有可能阻塞的 helper business 点都必须只执行上述固定上界的 poll，并在 poll 前后各 checkpoint：spawn/kernel publication、READY read、唯一 START write、RESULT read、EOF probe、成功路径 reap 与 pipe close。`HelperSpawner.spawn(..., max_wait_ns=...)` 每步只可返回 exact 已预发布 kernel 或 identity `PENDING`；stdout read 只可返回非空 bytes、exact EOF `b""` 或 `PENDING`，其中 `PENDING` 永远不等于 EOF。START write 必须是全帧原子的 `COMPLETE | PENDING`：`COMPLETE` 只表示 exact frame 已完整接受一次，`PENDING` 必须表示未提交任何部分，partial count、`None`、bool 或其他值都不能授权继续；terminate/close 同样只接受 `COMPLETE | PENDING`，reap 只接受 plain exit status 或 `PENDING`。`AttemptGate._commit_dns_start` 是 stop 与首个 START 的线性化点：cancel/expiry 先提交则 START 为零；DNS commit 先提交则本 attempt 至多发送这一个原子 START，随后 stop 只中断 bounded poll、触发 cleanup，绝不产生第二 START、retry 或预算退款。RESULT receipt 已 exact 证明 RESULT/EOF/reap(0)/close 后，还必须取得 Gate-issued、one-shot、exact-bound resolver completion claim 并由独立 Gate ledger postcondition 确认；该 proof 不只 gate coordinator 的 `PreparedResolverAttempt` 返回，也必须在 `build_resolution_set()` 内 gate 任意 direct `ResolutionSet` publication，因此任何内部直调都不能绕过 stop/completion 线性化点。stop 先赢该 claim 时禁止所有 Resolution/Prepared publication；claim 先赢只允许发布这一个 exact receipt-bound 离线 resolver outcome，不授予 B2b connect 或后续 wire authority。所有 caller-supplied synchronous lifecycle/cleanup observer 或 callback API 均从公开/可注入边界删除；commit、terminal 与 ref-release 只由 module-private ledger 查询独立证明，caller 不能用可能阻塞、no-op 或伪造返回的回调充当安全事实。

业务 cancel/deadline 只终止 business transition，不能阻止 cleanup，也不能把已经到期的 business wait slice 当作 cleanup authority。helper cleanup 必须使用独立的 cleanup-only 固定上界：每个 selected terminate/reap/close action 最多 8 个 poll，每个 poll 最多 50_000_000 ns；显式 `PENDING` 只允许在该界内再次 poll。上界耗尽、返回合同非法、异常或 action outcome 不确定时都不得提交 terminal；caller-owned `ResolverCleanupTicket` 必须保持 recoverable/nonterminal，并继续保留 helper、Attempt 与 credential recovery owner，既不能假报 `resources_terminal_proven`，也不能重放 outcome 不确定的 action。helper lifecycle terminal 还必须清掉 exact stop authority 的 context/ledger/session/deadline/token 强引用，并由 `AttemptGate` 的独立 helper-stop release observer exact 证明 authority terminal、七个 owner refs 全空、credential terminal 且 session 无 active owner；cleanup wrapper 的 true、lifecycle terminal 或 aggregate 三层 terminal 中任一单独成立都不能替代该 proof，也不能提前把 ticket 标记 terminal。该切片只冻结并离线验证 injected fake spawner/kernel 的 capability、poll、race、claim、release 与 cleanup 状态机；它不证明真实 OS 调用遵守时限。native executable/spawn shim、真实 process/pipe/select/kill/waitpid/reap/FD liveness、实际 DNS、numeric socket/connect 与 `getpeername()` peer/rebinding 证据全部保留给 W09-B2b，production spawner 在此之前继续零进程 fail-closed。

`main@341374b` 中的 W09-B2b Darwin native process foundation 这一层只推进真实 OS 进程边界，不启用 production resolver。`DarwinResolverProcessSpawner` 位于私有模块 `transport/_darwin_resolver_process.py`；import 与构造保持零 process/file/environment/network I/O，只有现有 lifecycle 提供 exact spawn request、publication sink 与 wait slice 后才进入 spawn。Darwin libc ABI 固定使用 `POSIX_SPAWN_CLOEXEC_DEFAULT | POSIX_SPAWN_SETSIGDEF | POSIX_SPAWN_SETSIGMASK`（`0x400c`），file actions 把一个 `AF_UNIX/SOCK_DGRAM` child endpoint 映射到 stdin、把两组 pipe 的 child ends 映射到 stdout/stderr，并关闭全部原始 fd；parent endpoints 为 nonblocking/non-inheritable，child stdin 保持 blocking，argv 与传入 envp 分别 exact 为冻结的两项 argv 及 `LANG=C, LC_ALL=C`。测试解释器可在启动后自行合成 Apple toolchain 环境键，因此当前证据只证明传给 spawn 的 envp 与 parent canary 不继承，不把 probe 的运行时 `os.environ` 冒充为 production executable 的 exact environment proof。

START record channel 的两端 `SO_SNDBUF/SO_RCVBUF` 都必须设置并读回为至少 `4 * MAX_START_FRAME_BYTES = 16384`；在 spawn 前还必须以 nonblocking 模式将一个不含 target 的 4096-byte canary 从该 exact parent endpoint 发送并由 exact child endpoint 单次 `recvmsg(MAX_START_FRAME_BYTES + 1)` 完整消费，拒绝 short send、ancillary data、`MSG_TRUNC/MSG_CTRUNC` 或容量/类型不符。canary 消费后 child endpoint 恢复 blocking，正式 helper 同样必须以一次 `recvmsg` 接受唯一 START，并拒绝空、超长、截断、ancillary 或第二条 record。parent 的单次 `os.write` 只有返回 exact frame length 才能提交 `COMPLETE`；`EAGAIN/EWOULDBLOCK/EINTR/ENOBUFS` 都表示本次整条 datagram 零提交并返回 `PENDING`，其他异常或非整帧返回进入 uncertainty 且不得重放。`poll(POLLOUT)` 只是提示，不能替代 write 结果。当前测试使用真实 `encode_start_frame()` 的最大合法输入，形成约 1.1 KiB record，已经跨过 Darwin `PIPE_BUF=512` 并由 child 以 byte-size/SHA-256 exact 证明单报文到达；这解决的是 native foundation 的真实 START message-boundary blocker，不是 production supervisor、DNS 或 Transport 证明。

该 kernel 继续使用 `poll` 而不是受 `FD_SETSIZE` 限制的 `select`，锁竞争直接返回 identity `PENDING`；terminate 前先 `waitpid(pid, WNOHANG)`，随后至多发送一次 `SIGKILL`；正常退出保存 `0..255`，signal death 保存不与正常退出码冲突的 `256 + signum`。stderr 在 READY/START/RESULT/reap 路径有界 drain，超过 4096 bytes 不能形成成功证明；IPC close 与 reap 可幂等观察，terminal status 已发布但尾部 bookkeeping 被一次异步异常打断时，下一次 reap/recovery 会补齐 `_waited_status/PID/PID cell` 清理。`waitpid` 前置 `in_flight`：若 destructive return 到 status publication 之间被打断，后续 terminate/reap 必须永久 poison，既不重放 waitpid 也不 signal 旧 PID；这保证 safety，但不能证明 child/zombie/resource liveness。真实本地 probe 覆盖最大合法 START、同帧幂等/不同帧拒绝、`ENOBUFS` 零提交、production 零接线、成功、READY/RESULT 阻塞取消、exit 7、早/晚 stderr overflow、parent environment canary 与高低位 inheritable-FD closure；合成 native-return fault 还验证非零 `posix_spawn` 写入任意正 PID cell 时零 kill/wait，以及未 attested spawn/wait outcome 永久 poison，但这些测试不解析 DNS，也不读取 credential 或执行外部 network I/O。

W09-B2b 对同步 spawn 的架构裁决固定为 **pre-secret persistent supervisor/broker**，不允许把 direct-process spawner 接入 production，也不把 thread/future 包装的前台 timeout 当作取消证明。应用级 supervisor 必须在 capture、credential backend read、secret buffer、request target 与 Attempt 创建之前启动；main application thread 唯一允许直接执行的同步 `posix_spawn` acquisition 是该 supervisor bootstrap，它明确位于 per-attempt `HelperSpawner.spawn(max_wait_ns)` 合同之外，不能宣称受 50 ms slice 硬约束。per-operation resolver child 的同步 spawn 只能由 supervisor 内与 control event loop 分离的 worker 执行，main request path 只能 bounded poll broker IPC。bootstrap 前必须预建 control/liveness/recovery owner；固定 READY、broker epoch 与 bundled executable identity 尚未验证时 Transport 必须不可用。bootstrap failure、late-success ownership uncertainty 或 shutdown 竞态不得在同一应用进程自动 retry 并继续发送；若不能证明 main 是 supervisor 的唯一 parent/reaper、supervisor 是 resolver child 的唯一 parent/signal owner/reaper，必须改用 launchd/XPC 等 OS-owned service，而不能依赖裸 PID。

当前完成的 **B2b-S2a** 只是上述目标的 Darwin-local development fixture foundation。受控仓库 fixture 经真实 `posix_spawn` 返回固定、单行、exact READY，parent 把其与 bootstrap/epoch/challenge/control/双向 liveness 六个 caller-bound ID、local policy digest、executable SHA、binding digest、PID 与 PPID 逐项匹配，再创建 S1 broker/session。bootstrap 在同一 interpreter instance 内 non-reentrant、one-shot；parent-death writer 与 supervisor-death writer 提供双向 EOF capability，`require_live`、exact reentry 与 shutdown 会被动观察 death/EOF、stderr 或 unsolicited control byte。这里没有 heartbeat、hang detection 或常驻 watchdog，fixture 的 READY 也是 child self-report 与 parent 预期 bytes 的一致性证明，不是独立的实际运行镜像 attestation。所谓 pre-secret 仅表示此 fixture 测试 API 不接收或读取 credential、secret、target、Attempt；所谓无网络仅描述受控 fixture 源码与测试行为，不是对 caller-selected generic executable 的 sandbox，也不证明真实应用已经按该顺序组合启动。

S2a local policy 把规范化绝对路径和预期 SHA-256 纳入 digest，并以 `lstat → open(O_NOFOLLOW) → fstat/hash → spawn 前后 path snapshot` 拒绝普通 symlink、owner/mode/link 与替换异常；它仍不能证明 kernel 实际执行的 image、shebang interpreter、bundle/code-sign designation、目录链或 ABA/TOCTOU。Python `open/pipe/native-init/posix_spawn/waitpid/close` 的 syscall return→owner publication 窗口只能由 preclaim/marker 诚实地永久 poison、阻止重放与假报 clean，不能保证未知 FD/native handle/child/zombie 已被回收。`main@341374b` 中的 caller-preheld native resolver owner 关闭了 injected syscall 下的 publication/sole-reaper foundation gap，但尚未绑定最终签名 image 与真实 process/signal/wait/FD vtable；生产仍需该 exact 绑定或 OS-owned durable service。所有 bootstrap API（包括 metadata）均禁止从 signal handler 调用。以上仍是 S2b/production 的硬边界，而不是 S2a 已解决能力。

当前完成的 **B2b-S2b-I1** 是独立、production-unwired 的 Darwin connection-peer dynamic-code identity foundation。预先创建的 socketpair 不能作为 child 身份：其 `LOCAL_PEERTOKEN` 仍指向创建 socketpair 的 parent。因此真实 peer 必须在 exec 后新连接 parent-owned 私有 AF_UNIX listener，parent 只从 accepted socket 的 kernel `LOCAL_PEERPID/LOCAL_PEERTOKEN` 取证，不信 child payload。opaque 32-byte audit token 只经 libbsm accessor 提取 PID/pidversion/euid；PID 必须等于 caller 给出的 expected spawn PID，pidversion 被保存在 proof，`proc_pidpath_audittoken` 必须返回 exact expected executable。随后以 `kSecGuestAttributeAudit` 获得 dynamic `SecCode`，先用 strict `SecCodeCheckValidityWithErrors` 对 exact identifier + CDHash requirement 校验，再从 signing information 独立核对 identifier、Team ID、20-byte unique/CDHash、main executable URL、static flags 与含 `CS_VALID` 的 dynamic status。所有 CF owner 在返回或异常前释放，外部只得到 factory-owned、不可复制序列化、content-addressed proof；raw token、path 与 signing material 不进入 safe metadata。

I1 的成功 fixture 是测试期间由 Apple clang 临时编译、以 ad-hoc + hardened runtime 签名的 native Mach-O；`codesign` CLI 只用于测试构建和提取预期 CDHash，运行模块不调用 CLI。I1 已证明 wrong PID/path/identifier/CDHash/euid/static-or-dynamic flags、stale generation、shebang 解释器替代与 socketpair creator-token 均 fail-closed；policy 要求 dynamic `CS_VALID/CS_FORCED_LV/CS_HARD/CS_KILL/CS_ENFORCEMENT/CS_RUNTIME/CS_SIGNED`，并拒绝 `CS_GET_TASK_ALLOW/CS_INVALID_ALLOWED/CS_KILLED/CS_DEBUGGED`。但 accepted connection 的历史 peer 身份不能提升为持续会话身份：真实 connect 后 fork+exec fixture 已证明另一 image 可以继承 FD 并写入后续字节。当前 `_darwin_suspended_identity.py` 与 `_darwin_process_events.py` 已把 `POSIX_SPAWN_START_SUSPENDED`、resume 前 dynamic validation、同一 audit token 的 reconnect proof，以及 `EVFILT_PROC NOTE_EXIT|NOTE_EXEC|NOTE_FORK` watcher 实现为 Darwin-local S2b-I2 foundation；native resolver owner 又提供 atomic publication/sole-reaper 的 injected foundation。它们使用 development artifacts，仍不提供 production authority：fixed bundle manifest/Team-signed native artifacts、watcher/identity/READY/epoch 与 fork-descendant 的真实 production vtable/owner 绑定尚未成立；不能证明这些边界时，production 必须继续 fail-closed或由 launchd/XPC 持有生命周期。

broker 侧 authoritative operation 的规范状态为 `reserved → attached → spawn_inflight → child_owned → ready → started → result_pending_terminal → terminal_attested → released`；cancel 在 spawn 返回前获胜时使用 `cancel_wait_spawn`。每个 snapshot 都携带单调 revision 与 content digest，并验证状态/事实/cleanup lattice；同 revision 不同 digest 必须隔离，旧 revision 只能被忽略。普通 protocol/event equivocation 只 poison exact operation，不能污染同 epoch 的健康 operation；但 START 已 claim 未 commit 时，任何被拒 mutator、cancel/terminal/result、epoch 或 liveness 中断都必须提升为全局 `OS_ACTION_UNCERTAIN`，因为既不能证明零发送，也不能安全重发。active identity、broker epoch/liveness 或 OS action outcome 不确定会使 parent/business authority 全局失信；broker 对已知 exact late child 仍只保留 cleanup-only authority，若 poison 时某个 terminate/reap/close 已 claim，则该 exact action outcome 必须冻结为不可重放。released tombstone 不得回退。在 stable exact epoch/operation/digest 下单纯 control ACK/response 丢失只是 parent 观察不确定，不能伪造 broker poison 或 terminal。

parent proxy 的观察状态独立冻结为 `attach_unknown / attached_unarmed / arm_unknown / active / cancel_not_attested / cancel_latched_wait_terminal / terminal_attested / release_not_attested / released / poisoned`。`RESERVE` 只建立绑定 `broker_epoch + operation_id + lifecycle_id + publication_id + spawn_request_digest` 的零-child record；每个 broker factory 必须同时发行且只承认自己的 exact parent-session 对象，exact `RESERVED` attestation 只能被该 session 消费，并唯一生成带 `proxy_id` 的 proxy/publication ledger。publication proof 同时绑定 exact Python proxy、exact publication-ledger 对象、reservation digest、broker owner 与 binding；相同 ID/digest 的另一 broker、session 或 proxy 不能先占、借用或重放。这里的对象 identity 是 S1 本地 capability，不是跨进程可序列化 wire proof。parent 必须先把 exact proxy kernel 发布到现有 lifecycle ledger，并由 module-private observer 证明 attachment，再发送显式 `ATTACH`；只有 ATTACH attested 后才可发送 `ARM`。publication false/no-op/exception/observer uncertainty 必须保持零 child。control-channel 的 `PENDING` 或 ACK loss 只允许重新投递调用方缓存的 exact control frame：`frame_id`、`frame_digest` 与 canonical bytes 必须全部相同；channel/broker 按同一 binding 去重，只返回缓存 ACK/tombstone 或 bound STATE evidence，不得再次执行逻辑 mutation 或 OS action。同一 ID 携带不同 digest/bytes 必须 fail-closed。该例外只覆盖 `RESERVE/ATTACH/ARM/CANCEL/RELEASE` control frames；START datagram、numeric connect、TLS handshake 或 HTTP request bytes 在 commit/write outcome 不确定后均禁止重放。ATTACH/ARM ACK 丢失分别留在 `attach_unknown`/`arm_unknown`，只有 exact-frame 重投或 bound query 取得 attestation 才可前进；cancel/release 也只能重投各自 exact cached frame 或查询相同 operation。main 永远不得取得 resolver PID。

B2b 的 published `HelperKernel` 必须解释为 durable、zero-child-capable 的 reserved-operation/recovery proxy：publication 只证明 recovery ownership 已可恢复，不证明 resolver child 已创建。每次 `HelperSpawner.spawn(..., max_wait_ns=...)` 仍必须在该 slice 内返回；首次调用可以 `RESERVE`、发布 proxy 后返回 `PENDING`，后续调用必须复用 exact operation，禁止重复 reserve/publication。只有 exact `ARM` 获 broker attestation 后才可向 coordinator 返回该 proxy；它不等待同步 `posix_spawn` 完成，READY 仍由 proxy 的后续 `read_stdout()` bounded poll 到达。当前 `_resolver_supervisor_wire.py` 与 `_resolver_supervisor_proxy.py` 已把该语义适配实现为 canonical-wire、in-memory/injected 的 S3 local/offline foundation；它没有真实 attested production channel、signed supervisor 或 native ownership，因此 production 仍不可用。

supervisor 的 control event loop 与执行同步 child spawn 的 worker 必须分离，并由唯一 event owner 线性化 `ARM/CANCEL/SPAWN_DONE/START`。cancel 必须先不可逆 latch；若 cancel 先于 spawn completion，late child success 必须先被 exact operation record 接管，再由 supervisor 至多一次 kill、exact `waitpid(exact_pid, WNOHANG)` 与 close，禁止 READY/START；若 spawn completion 先赢，必须先冻结 child ownership，再允许 cancel。supervisor 禁止 `waitpid(-1)`；`ECHILD`、unexpected PID、kill/reap uncertainty 均 poison，`ESRCH` 不能替代 reap proof。START 与 CANCEL 同样由一个 event owner 排序：cancel 先赢则 START 为零，START 先赢也至多一个 record。`_resolver_supervisor_async.py` 已把 `claim_start → exact single-datagram write → commit_start` 放进不可交错 event-owner critical section，并覆盖 async spawn/late completion/cleanup；`_darwin_resolver_owner.py` 与 `_darwin_resolver_async_adapter.py` 又把该合同接到 caller-preheld native owner 的 signal/wait/FD lanes。claim 后、commit 前的任何中断或冲突仍一律全局 `OS_ACTION_UNCERTAIN`。这一接点仍使用 injected vtable，尚不是绑定签名 artifact 的 native production owner。

parent-side proxy 的 `terminate()` 只提交 broker-bound cancel control frame，实际 signal 只能由 supervisor 执行；`reap()` 只读取 supervisor 已缓存的 terminal attestation，不能调用 OS `waitpid`。`cancel_not_attested` 只允许 exact cached CANCEL frame re-delivery 或 bound query；`cancel_latched_wait_terminal` 只允许 read-only terminal observation。parent cleanup 独立建模为 `idle / polling / cleanup_waiting_supervisor / terminal`，broker action phase 则为 `none / terminate_required / terminate_claimed / reap_required / reap_claimed / close_required / close_claimed / complete`；claim-before-action 与 exact action id 不得被解释为重做 OS action 的授权。若一次既有 8×50 ms cleanup 窗口只得到 stable epoch/identity 下的可信 `PENDING`，ledger 必须进入 `cleanup_waiting_supervisor` 而不是 `cleanup_failed`；后续 `ResolverCleanupTicket.retry_cleanup()` 只能继续查询/观察该 exact operation。S4 local/offline layer 与 native owner foundation 已执行这些排列；production 仍需将 exact signal/wait/close/sole-reaper vtable 绑定最终签名 supervisor/helper，并取得真实 action/liveness proof。

proxy 的 `close_pipes()` 在 terminal attestation 与 operation release ACK/tombstone 前必须返回 `PENDING`，且绝不能关闭 supervisor session 的全局 control/liveness/terminal-observer channel；terminal 已证明后也只能关闭 operation-local data endpoints，全局 channel 始终由 supervisor session owner 管理。当前 S3/S4 integration、`_resolver_output_cache.py` 与 native resolver owner/async adapter 已覆盖 `cleanup_waiting_supervisor`、`reap PENDING ×8 → late terminal → retry/release` 和 READY/RESULT/EOF 先 durable publication、后 exact ACK；caller-preheld buffer 关闭了 Python destructive read 的 return→publication foundation gap。production 仍须用最终签名 artifacts 所绑定的真实 pipe/read/ACK vtable 取得同一证明。broker boot epoch 改变或 broker 在 active operation 存在时死亡，main 必须全局 poison且不得以重启 broker 伪造 reap/FD terminal。terminal attestation、operation release ACK/tombstone、durable output ACK 与 helper-stop release proof 全部成立前，不得释放 Attempt、credential 或 aggregate recovery refs。

因此 `main@341374b` 已包含 B2b S1–S6 及其无生产身份可构建 foundations：严格 helper/DNS、caller-preheld native resolver/output owner 与 async adapter、Keychain staged-source bridge、numeric/TLS owner、原生 C-to-C descriptor transfer 和 exact-peer 合同。三轮独立退出审查最终 P0/P1=0，但这不是 production completion：S2c `PRODUCTION_STARTUP_INTEGRATION_AVAILABLE`、S4 durable-output production integration、S6 AttemptGate/opaque-socket integration 等 gate 均保持 false，S7 也未启用。Production 仍缺 fixed Team-signed app/supervisor/helper/native artifacts 与最终 bundle/signing/entitlement facts、绑定这些 artifacts 的真实 process/DNS/socket/SSL/Security.framework vtable、真实 control/liveness/Keychain/transport evidence、app cutover 与独立 activation/acceptance 授权。上述门槛全部关闭前，`ResolverHelperLauncher.production()` 必须继续使用 `FailClosedProductionHelperSpawner`，W09-B2b、W09-B2 与 M5 均不得宣称 production complete。
...
claimed attempt 在成功、普通异常、`BaseException`、cancel 与 cleanup fault 下都必须由同一 cleanup owner exactly-once 推进。资源顺序固定为先证明 helper/process/pipe terminal，再终结 Attempt/Gate，最后关闭并 best-effort 清零 credential handle；当前 B2b–B4 foundations 已把 selector/raw socket/TLS socket 纳入所有权与负向矩阵，并新增 opaque native numeric/TLS owner 及 C-to-C transfer，预算永不回退。这些 owner 仍使用 injected vtable，production 必须再绑定真实 socket/system SSL implementation 与签名 artifact。普通成功与显式 `PreparedResolverAttempt.close()` 在完整 terminal 前不得宣称完成；若协调失败同时遇到真实 helper action outcome uncertainty，则只能在调用方已经预持 exact cleanup ticket 时原样抛出主异常并保持 recoverable，不能假报 terminal、提前释放 Attempt/secret owner 或重放未知动作。公开 ticket retry 不暴露 raw cleanup error，只返回 false 并保留可重试状态；产品错误边界仍必须把 cleanup 错误归一为不含 raw cause/context 的 typed error，ticket metadata 同样不得暴露 cleanup raw error。

`native/darwin_numeric_owner.c`、`native/darwin_tls_owner.c` 与 `native/darwin_owner_transfer.h` 把 numeric FD 与 TLS owner 设计为互不重叠的 opaque identity，并只允许 exact native C-to-C descriptor transfer；Python `_darwin_transport_adapter.py` 不取得 raw PID/FD。caller-preheld owner slot 在 syscall/SSL action 前发布，native snapshot 同时绑定实际 write bytes、digest 与 callback，CAS deinit/accept 和 release/acquire publication 关闭 transfer race；successful/uncertain descriptor-free retire 使用不可回退 tombstone，容量耗尽 fail-closed。alias mutation、adopt publication、double-failure retire、deinit/accept race、return-gap recovery 与 70 轮 capacity 回归均已覆盖，但真实 socket/system SSL vtable 仍未接线。

`scripts/build_w09_native.py` 以固定 Apple clang、最小环境、严格 warning/analyzer flags 和内容寻址 source/header snapshots 构建四个 arm64 dylib；snapshot/header/manifest 原子发布为 `0400`，dylib 为 `0500`，全部 `nlink=1`，install name 固定为 `@rpath/<final-name>`。Python 3.12.13 与 3.10.20 的真实构建逐字节一致。当前产物仅为 linker ad-hoc、无 TeamIdentifier；manifest v2 明确 `production_signed=false` 与 `production_authority=false`，因此可复现构建只证明 development artifact integrity，不提供生产签名或运行 authority。

`_production_readiness.py` 的 manifest v5 是 fail-closed exact exit contract，不是 authority issuer：它把 exact TLS policy、Keychain service/account/access-group 与 resolver mapping、native capability interface requirements、app application-identifier/Team/keychain-access-groups entitlement、`Contents/MacOS` entrypoint SHA 和完整 transport binding 纳入同一证据 DAG。当前 v5 不消费 native build manifest v2、四个 dylib 的路径/摘要或 install-name；这些仍是独立 development evidence，未来若作为 S7 正向门禁必须升版并绑定最终 artifact/build-manifest digest。`APPLICATION_TRANSPORT_CUTOVER` v2 使用 `manifest content digest → cutover subject → final manifest` 两层绑定，并要求 content-digest-bound generic attestation 与专用 fresh probe 同时通过；旧 v1/version-only、generic-only substitution、stale/cross-manifest/same-manifest replay 与 tamper 均失败。probe 对当前 request/manifest 与 process-local one-shot identity challenge 作 exact 回显，challenge 不进入持久化 digest 或 safe metadata。由于本机没有有效正式签名身份和最终 app bundle，readiness/availability flags 保持 false，所有 assessment 继续包含 `production_authority_unavailable`。

版本化 `TransportLimits` 第一版冻结为：request body 至多 8 MiB、response body 沿用 `MAX_PROVIDER_RESPONSE_BYTES=2 MiB`、response headers 至多 64 KiB、至多 128 个字段、单行至多 8 KiB、DNS raw IPC 至多 16 KiB/candidates 至多 32 个、chunk count 至多 4096 且 framing metadata 至多 64 KiB。W09-B0 冻结合同，W09-B1 实现 CredentialHandle/resolver；当前 B2b/B3/B4 已完成本地/离线与无生产身份可构建 foundations，包括 DNS/all-result/peer、native resolver/output、Keychain、numeric/TLS/C-to-C transfer、exact TLS/HTTP `send_once`、fake-edge Transport、reproducible native build，以及 fault/race/cleanup/readiness v5。production 仍需最终 Team-signed artifacts、真实系统 vtable/entitlement/app integration、真实 macOS resolver/transport evidence 与独立 activation authority；因此 B2b/B3/B4、W09-B、M5 和真实 API 路径仍不得宣称 production complete。
...
4. **opt-in live smoke**：只在保留已完成的 W09-A 证据、关闭 W09-B production exit gates、完成 W10 与完整离线负向矩阵并取得另行明确授权后，每个 profile 使用固定非敏感合成题图做单次、无自动 retry 的真实 API 验证；不得在普通单测中运行；
...
- `M6`：仅在保留已完成的 W09-A 证据、关闭 W09-B production exit gates、完成 W10 与完整离线负向矩阵后，经另行明确授权，用仓库内固定、非敏感合成题图执行一次、无自动 retry 的 opt-in GLM live smoke；
...
**当前实现状态（2026-09-05）**：远端 `main@341374b` 已完成并推送 M0–M4、M5/W08 与 W09 的全部 local/offline 范围。当前未提交工作区已完成 W10 的 local/offline reference executor、Provider-neutral Adapter 接口、目录驱动 Egress Adapter 选择、完整 gate 顺序、capture/result claim、caller-preheld recovery、resolver return attestation、取消/single-flight 与 cleanup-only finalization；W08–W10 的 Python 3.12.13/3.10.20 定向集分别 341/341、完整离线 suite 分别 1341/1341 通过，独立终审 P0/P1/P2=0。所有测试使用合成 PNG 和 injected fake owner/edge，没有真实截图、secret、进程、DNS/socket/HTTP 或 Provider API。W10 production factory、W09 production availability/readiness、S7 与 CLI/app 入口仍全部 disabled。Production W09/M5 仍需 fixed Team-signed artifacts、最终 bundle/signing/entitlement facts、绑定真实系统 vtable、真实 control/liveness/Keychain/transport evidence、app cutover，以及独立 activation/acceptance 授权。因此 W09/M5 production 继续 `blocked`，W10 只在 local/offline 范围 `complete`；GLM 仍为 `experimental`，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、live/eval/E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

W10 可以基于 W09 的 local/offline foundations 独立完成，且当前已收口；W09 production closure 不是 W10 离线开发的前置条件，但仍是 W11 真实调用的硬门槛。下一步先取得外部签名身份、最终 artifacts、真实系统绑定与授权证据并关闭 W09 production gates；随后仍须另行明确授权，才可在 M6/W11 用固定非敏感合成图做一次、无自动 retry 的真实 API smoke。真实用户截图只能在该 smoke 完成后的 M7/W12 接入。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。
...
│   ├── resolver.py             # W09-B2a：offline coordinator / lifecycle contracts
│   ├── http.py                 # W09-B2a：PreparedResolverAttempt / cleanup anchor
│   ├── _resolver_*             # W09-B2b S3–S5 local/offline supervisor/output/DNS
│   ├── _darwin_keychain_source.py # W09：caller-preheld Keychain source bridge
│   ├── _darwin_resolver_*      # W09：native resolver owner + async adapter
│   ├── _darwin_numeric_owner.py # W09：opaque numeric owner binding
│   ├── _darwin_tls_owner.py    # W09：opaque TLS owner binding
│   ├── _darwin_transport_adapter.py # W09：exact native C-to-C transfer adapter
│   ├── _darwin_*               # W09-B2b identity/process/watcher foundations
│   ├── _numeric_connect.py     # W09-B2b S6 local/offline numeric exact-peer
│   ├── _exact_tls.py           # W09-B3 local/offline exact TLS policy
│   ├── _exact_http1.py         # W09-B3 network-free exact HTTP/1.1 codec
│   ├── _exact_transport.py     # W09-B3 fake-edge single-request composition
│   ├── _production_readiness.py # W09-B4 readiness v5 fail-closed exit inventory
│   └── native/                 # W09 resolver/numeric/TLS owners + transfer ABI
├── runtime/
...
仓库级 `scripts/build_w09_native.py` 是 W09 reproducible development native build 入口。目录是目标边界，不要求一次性创建空文件；只在对应阶段实际实现模块。

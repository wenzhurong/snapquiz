...
> **状态**：v3 实现基准。本文描述目标架构；远端 `main@5fe5eab` 已完成 M0–M4、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a。当前未提交工作区已经完成 W09 的 local/offline 与全部无生产身份可构建 foundations：B2b S1–S6、caller-preheld native resolver/output owner 与 async adapter、Keychain staged source→CredentialResolver bridge、opaque numeric/TLS owner 与原生 C-to-C transfer、B3 exact TLS/HTTP/1.1 与 fake-edge single-request Transport、reproducible native build，以及 B4/readiness v5 exact exit inventory。三轮独立退出审查最终 P0/P1=0，Python 3.12.13 与 Python 3.10.20 完整离线 suite 均为 1235/1235；跨解释器实际构建的 4 个 dylib 逐字节一致，但只有 linker ad-hoc 签名且无 TeamIdentifier。它们均刻意未接入 production，不能授权真实发送。Production W09/M5 阻塞于 fixed Team-signed app/supervisor/helper/native artifacts、最终 bundle/signing IDs 与 Keychain entitlements、绑定这些 artifacts 的真实 process/DNS/socket/SSL/Security.framework vtable、真实 control/liveness/Keychain/transport evidence、S7/app cutover，以及另行明确的 activation/acceptance 授权。相应 production flags 继续为 false，`ResolverHelperLauncher.production()` 与 CLI 继续 fail-closed；W09-B2b、W09-B3、W09-B4、整个 W09-B 与 M5 的 **本地可完成范围 complete，production 范围 blocked**，不得合并成 production complete 声明。后续顺序保持 W10 multimodal pipeline → W11 单次真实 API smoke → W12 真实截图纵切。交付顺序、状态与逐项验收见 [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)。
>
> **已定方向**：模型能力分为两条推理通道：
>
...
- **W09-B2b 本地可完成范围 complete，production blocked（当前未提交）**：既有 native process/message-IPC、S1 pure/local supervisor、S2a fixture bootstrap 与 S2b-I1 connection-peer identity 之外，工作区已加入 S2b-I2 suspended identity/process watcher、S2c pre-secret startup composition、S3 canonical wire + durable zero-child proxy、S4 async event owner/output cache/native resolver owner adapter、S5 strict local helper/DNS，以及 S6 numeric/TLS native owner、C-to-C transfer 与 exact-peer foundation。它们分别冻结本地合同与失败边界，但 production startup、真实系统 vtable、final signed artifacts 和 app integration flags 都保持 false；
- **W09-B3/B4 本地可完成范围 complete，production blocked（当前未提交）**：`_exact_tls.py`、`_exact_http1.py` 与 `_exact_transport.py` 已组成 exact TLS/HTTP/1.1、zero redirect/retry 的 fake-edge single-request Transport；Keychain bridge 与 native build 分别提供独立 source/build evidence，readiness v5 则冻结 capability interface、app/entitlement 与 cutover 的 exact exit shape；B4 已覆盖 fault/race/cleanup 与 fail-closed production exit inventory。三轮独立退出审查最终 P0/P1=0；Python 3.12.13 与 Python 3.10.20 完整离线 suite 均为 1235/1235，但这些证据不构成 production completion；
...
因此当前未提交工作区已实现 B2b S1–S6 及其无生产身份可构建 foundations：严格 helper/DNS、caller-preheld native resolver/output owner 与 async adapter、Keychain staged-source bridge、numeric/TLS owner、原生 C-to-C descriptor transfer 和 exact-peer 合同。三轮独立退出审查最终 P0/P1=0，但这不是 production completion：S2c `PRODUCTION_STARTUP_INTEGRATION_AVAILABLE`、S4 durable-output production integration、S6 AttemptGate/opaque-socket integration 等 gate 均保持 false，S7 也未启用。Production 仍缺 fixed Team-signed app/supervisor/helper/native artifacts 与最终 bundle/signing/entitlement facts、绑定这些 artifacts 的真实 process/DNS/socket/SSL/Security.framework vtable、真实 control/liveness/Keychain/transport evidence、app cutover 与独立 activation/acceptance 授权。上述门槛全部关闭前，`ResolverHelperLauncher.production()` 必须继续使用 `FailClosedProductionHelperSpawner`，W09-B2b、W09-B2 与 M5 均不得宣称 production complete。
...
**当前实现状态（2026-09-04）**：远端 `main@5fe5eab` 已完成 M0–M4、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a。当前未提交工作区已经完成 W09 的 B2b S1–S6、caller-preheld native resolver/output owner 与 async adapter、Keychain staged source→CredentialResolver bridge、opaque numeric/TLS owner 与 C-to-C descriptor transfer、B3 exact TLS/HTTP/1.1 + fake-edge single-request Transport、reproducible native build，以及 B4/readiness v5 exact exit inventory。三轮独立退出审查最终 P0/P1=0；Python 3.12.13 与 Python 3.10.20 完整离线 suite 均为 1235/1235，B4 均为 7/7，native build 均为 16/16，numeric/TLS/transfer 联合回归均为 112/112。跨解释器实际构建的四个 dylib 逐字节一致、固定 `@rpath`、输入/manifest `0400`、输出 `0500`、均 `nlink=1`，但只有 linker ad-hoc 签名且无 TeamIdentifier。这些证据使 W09 的本地/离线与无生产身份可构建范围 complete，不能宣称 production complete。所有 production availability/readiness flags 为 false，`ResolverHelperLauncher.production()` 与 CLI 继续 fail-closed，S7 未启用。Production W09/M5 仍需 fixed Team-signed app/supervisor/helper/native artifacts、最终 bundle/signing IDs 与 Keychain entitlements、绑定这些 artifacts 的真实 process/DNS/socket/SSL/Security.framework vtable、真实 control/liveness/Keychain/transport evidence、app cutover，以及独立 activation/acceptance 授权。因此 W09-B2b、W09-B3、W09-B4、整个 W09-B 与 M5 的本地可完成范围为 `complete`，production 范围为 `blocked`；GLM 仍只有 `experimental` binding，应用仍被冻结，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、Provider API live smoke 或真实 macOS E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

`M1–M4`、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 已在远端完成；当前工作区的 W09 本地可完成实现与独立审查也已收口，production 集成明确标记为 `blocked`。产品开发的下一软件工作包是 W10 multimodal pipeline；W09 production closure 则需要外部签名身份/最终 artifacts/真实系统绑定与授权证据，不能靠继续增加 fake 能力关闭。首次真实 API 只能在 M6/W11，且要求 W09 production exit gates 与 W10 离线验收都已通过，再另行明确授权、只用固定非敏感合成图、只调用一次并关闭自动 retry；真实用户截图只能在该 smoke 完成后的 M7/W12 接入。因此顺序严格保持 **W10 → W11 真实 API → W12 真实截图**。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。

...
> **状态**：v3 实现基准。本文描述目标架构；M0–M4、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 已完成。W09-B2a 已覆盖 owner/address/helper、唯一 offline coordinator、proof-bound RESULT receipt/`ResolutionSet`、completion attestation、capability/recovery sealing、caller-owned `ResolverCleanupTicket`，以及贯穿 injected helper 阻塞点的 monotonic deadline/cancellation、bounded poll、completion-publication gate 与独立终态释放证明；本次收口证据为完整离线 580/580、W09 275/275，独立终审无剩余 P0/P1。整个 W09-B2 与 M5 仍为 `in_progress`，下一工作包 W09-B2b 为 `pending`；这仍不代表 production resolver 或 v3 用户链路已经可用。交付顺序、状态与逐项验收见 [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)。
>
> **已定方向**：模型能力分为两条推理通道：
>
...
- **W09-B2a complete（M5 子项）**：`62c53c1` 的第一批把 AttemptGate claim 收紧为 caller-owned `active → claiming → io_claimed`，加入 exact terminal-guard/DNS START proof、冻结地址策略与 injected helper 生命周期；`1ac6fca` 的第二批又加入唯一 offline coordinator、helper wire v2、factory-only RESULT receipt、单次 ledger-anchored `ResolutionSet`，以及 credential/READY return-publication recovery；`d455c8e` 再把 stdout EOF、无第二帧、plain exit status 0、single reap 与 parent pipe closure 设为 receipt 的联合发行门槛；`5468b52` 已由 launcher factory 生成并 snapshot 四个 role proof，冻结构造时 request/spawner，移除 caller UUID 与 public lifecycle bypass，要求 kernel 在 spawner 返回前进入 ledger，并以 launch owner、strong registry、cleanup-only snapshots、独立 commit/terminal observer 与 state recovery 覆盖 alias/tamper/callback/no-op fault；`8047b9d` 再提供调用前预持、三层聚合且 cleanup-only 的 `ResolverCleanupTicket`，异常后不会丢失恢复入口，也不会把 action uncertainty 假报为 terminal。本次收口再加入 Gate-issued exact `HelperStopAuthority`、50 ms helper poll、stale/no-op `HelperWaitSlice` postcondition、direct `ResolutionSet` publication gate、每动作 8×50 ms cleanup 与独立 helper-stop release proof；production spawner 仍零进程 fail-closed。`5468b52` 历史基线的完整离线测试为 532/532，W09 为 227/227，核心五模块 172/172；`main@8047b9d` 历史基线为完整离线 557/557、W09 252/252、核心五模块 197/197、coordinator 82/82。本次收口的独立证据为完整离线 580/580、W09 九模块 275/275、核心五模块 198/198（attempt 36/36、context 12/12、address 17/17、lifecycle 43/43、coordinator 90/90），且 101 个 Python 文件通过 3.10 AST grammar、`compileall` 与 `git diff --check`；独立终审无剩余 P0/P1。M5 与整个 W09-B2 继续 `in_progress`，W09-B2b `pending`；
- **尚不可用**：没有 production credential source、满足 no-fork/close-all-FD 证明的 native helper launcher、真实 Quartz/TCC 验收、真实 display/选区/截图 source、NSPanel 人机预览、numeric connect、HTTP/TLS/DNS/连接 peer、完整 I/O 终态清理或任何 live/eval/E2E 证据，因此当前仍没有可执行的解题用户路径；Registry 中的 `experimental` 只是 exact binding 状态，不代表应用链已可运行，更不能称为 `supported`；
- legacy `Config`、parse/notify 与 `AnswerResult` 仍留在源码树中，但被 M0 产品入口隔离；旧布尔权限 helper 已永久 fail-closed，新 W06–W09-B2a 链尚未接入应用。当前默认离线链只使用 fake credential/helper source，不截图、不构造 SDK 或联网；production credential source、真实 resolver、numeric connect、连接 peer 与 rebinding 防护仍属于后续 W09-B2b。
...
claimed attempt 在成功、普通异常、`BaseException`、cancel 与 cleanup fault 下都必须由同一 cleanup owner exactly-once 推进。资源顺序固定为先证明 helper/process/pipe terminal，再终结 Attempt/Gate，最后关闭并 best-effort 清零 credential handle；后续 B2b–B4 还须把 selector/raw socket/TLS socket 纳入同一所有权链，预算永不回退。普通成功与显式 `PreparedResolverAttempt.close()` 在完整 terminal 前不得宣称完成；若协调失败同时遇到真实 helper action outcome uncertainty，则只能在调用方已经预持 exact cleanup ticket 时原样抛出主异常并保持 recoverable，不能假报 terminal、提前释放 Attempt/secret owner 或重放未知动作。公开 ticket retry 不暴露 raw cleanup error，只返回 false 并保留可重试状态；B2b–B4 面向产品的普通错误边界仍必须把 cleanup 错误归一为不含 raw cause/context 的 typed error，ticket metadata 同样不得暴露 cleanup raw error。
...
**当前实现状态（2026-09-02）**：M0–M4、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 已完成。B2a 的 ticket factory attach/bind 及每个 helper/credential/attempt/guard 风险边界都有独立 normal-return postcondition；cleanup 固定按 helper → Attempt/Gate → credential 推进，只信 ledger terminal observer，不信 recovery 返回值。public retry 在 issued/coordinating 时不取消，在瞬时 wrapper fault 后恢复 recoverable；`cleanup_failed` 不重放 outcome 不确定的 helper action。三层资源证明完成后先冻结 `resources_terminal_proven`，引用释放或其 observer 失败只重试 bookkeeping；terminal credential state 只保留 primitive owner proof。`publish_recoverable` 是 return 前的 cleanup ownership 线性化点，并发 ticket cleanup 可以使返回的 Prepared 已同步为 closed。本次收口已实现 Gate-issued exact `HelperStopAuthority`、request/session effective deadline、50 ms 前后 checkpoint poll、stale/no-op `HelperWaitSlice` 独立 postcondition、DNS START/stop 与 completion/stop 线性化、direct `ResolutionSet` publication gate、每个 cleanup action最多 8×50 ms，以及 helper-stop terminal/ref-release 独立证明；所有 caller-supplied synchronous lifecycle/cleanup observer API 已删除。验证证据为完整离线 580/580、W09 九模块 275/275、核心五模块 198/198（attempt 36/36、context 12/12、address 17/17、lifecycle 43/43、coordinator 90/90），101 个 Python 文件通过 3.10 AST grammar、`compileall` 与 `git diff --check`，独立终审无剩余 P0/P1。本机实际运行时仍为 Python 3.12.13，真实 Python 3.10 runtime suite 证据仍缺失。这些数字不构成 production/B2b 证明；当前路径仍只使用 fake source/kernel，保持零真实环境凭据、零真实进程、零 client、零 DNS/socket/HTTP。production credential source、cancellable native strict-spawn helper、真实 process/DNS/numeric connect/peer、B3 exact HTTP/TLS 及 B4 全资源矩阵仍不存在，因此整个 W09-B2 与 M5 继续 `in_progress`，下一步 W09-B2b 为 `pending`。GLM 仍没有 VerificationRecord，只派生 `experimental`；应用仍被有意冻结，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、Provider API、live smoke 或真实 macOS E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

`M1–M4`、M5/W08、W09-A、W09-B0、W09-B1 与 W09-B2a 已经完成；下一步进入仍为 `pending` 的 B2b native production resolver、numeric single-connect 与真实 peer/rebinding proof，随后依次完成 B3 exact Transport 与 B4 完整矩阵。首次真实 API 只能发生在 M6/W11，且需要另行明确授权、只用固定非敏感合成图、只调用一次并关闭自动 retry；真实用户截图只能在该 smoke 完成后的 M7/W12 接入。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。

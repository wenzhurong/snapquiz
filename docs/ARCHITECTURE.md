...
> **状态**：v3 实现基准。本文描述目标架构；远端 `main@1ac6fca` 已完成并推送 M0–M4、M5/W08、W09-A、W09-B0、W09-B1，以及 W09-B2a 的 owner/address/helper、唯一 offline coordinator、proof-bound RESULT receipt/`ResolutionSet` 与 return-publication recovery 离线增量。当前未提交工作区继续推进 RESULT completion attestation；W09-B2a 与 M5 仍为 `in_progress`，这仍不代表 v3 用户链路已经可用。交付顺序、状态与逐项验收见 [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)。
>
> **已定方向**：模型能力分为两条推理通道：
>
...
- **M5/W09-B2a in progress（第二批已推送至 `main@1ac6fca`，completion attestation 当前未提交）**：`62c53c1` 的第一批把 AttemptGate claim 收紧为 caller-owned `active → claiming → io_claimed`，加入 exact terminal-guard/DNS START proof、冻结地址策略与 injected helper 生命周期；`1ac6fca` 的第二批又加入唯一 offline coordinator、helper wire v2、factory-only RESULT receipt、单次 ledger-anchored `ResolutionSet`，以及 credential/READY return-publication recovery。当前未提交增量把 stdout EOF、无第二帧、plain exit status 0、single reap 与 parent pipe closure 设为 receipt 的联合发行门槛；外部 reap/close 在调用前由 ledger 预占，失败或 return-then-raise 后不会重复动作。production spawner 仍零进程 fail-closed；完整离线测试 474/474，W09 为 169/169。B2a 仍缺 factory-generated READY/lifecycle proof 与 public bypass 封口、deadline/cancellation 及 cleanup-failure recovery ticket/retry，因此尚未 complete；
...
**当前实现状态（2026-09-01）**：M0–M4、M5/W08、W09-A、W09-B0、W09-B1，以及 W09-B2a 的 coordinator/receipt/ledger binding 与 return-publication recovery 已进入远端 `main@1ac6fca`；M5 与整个 W09-B2a 仍在进行。当前未提交工作区新增 RESULT completion attestation：receipt v2 exact 绑定 stdout EOF、无第二帧、child exit status 0、single successful reap 与 parent pipe closure，ledger 在外部 reap/close 前 claim action，禁止异常后重复动作。完整离线测试 474/474，W09 为 169/169，当前共 102 个 Python 文件；本机实际运行时仍为 Python 3.12.13，真实 Python 3.10 runtime suite 证据仍缺失。这些路径只使用 fake source/kernel，保持零真实环境凭据、零真实进程、零 client、零 DNS/socket/HTTP。production credential source、factory-generated READY/lifecycle proof 与 public bypass 封口、deadline/cancellation、cleanup-failure recovery ticket/retry、cancellable native strict-spawn helper、真实 process/DNS/numeric connect/peer、B3 exact HTTP/TLS 及 B4 全资源矩阵仍不存在，因此 W09-B2a/W09-B2/M5 不得升级为 complete。GLM 仍没有 VerificationRecord，只派生 `experimental`；应用仍被有意冻结，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、Provider API、live smoke 或真实 macOS E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

`M1–M4`、M5/W08、W09-A、W09-B0 与 W09-B1 已经完成；W09-B2a 的 owner/address/helper、唯一 offline coordinator、RESULT receipt/`ResolutionSet` ledger binding 与 return-publication recovery 已推送，RESULT completion attestation 已在当前未提交工作区实现。下一步用 factory-generated READY/lifecycle capability 封住 public bypass，再补齐贯穿 helper 的 deadline/cancellation 与 cleanup-failure recovery ticket/retry；随后完成 B2b native production resolver、numeric single-connect 与真实 peer/rebinding proof，再依次完成 B3 exact Transport 与 B4 完整矩阵。首次真实 API 只能发生在 M6/W11，且需要另行明确授权、只用固定非敏感合成图、只调用一次并关闭自动 retry；真实用户截图只能在该 smoke 完成后的 M7/W12 接入。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。

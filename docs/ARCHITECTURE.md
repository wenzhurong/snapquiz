...
> **状态**：v3 实现基准。本文描述目标架构；远端 `main@543d390` 已完成并推送 M0–M4、M5/W08、W09-A、W09-B0 与 W09-B1 的离线 Registry、Planner、Consent、Capture、纯 Adapter、Egress/session、runtime authority、CredentialHandle/frozen-binding resolver 与 secret lifecycle。当前未提交工作区正在推进 W09-B2a 的 owner-bound attempt、纯地址策略和可注入 helper 生命周期原语；这仍不代表 v3 用户链路已经可用。交付顺序、状态与逐项验收见 [`IMPLEMENTATION_PLAN.md`](./IMPLEMENTATION_PLAN.md)。
>
> **已定方向**：模型能力分为两条推理通道：
>
...
- **M5/W09-B2a in progress（当前工作区，未提交）**：AttemptGate 已把无主 claim 收紧为 caller-owned `active → claiming → io_claimed`，并加入 exact terminal-guard 与 one-shot DNS START authority proof；纯 `address_policy` 冻结 IANA-2025-10-09 表、16 KiB/32 条上限、整组拒绝、确定性去重排序、AttemptPermit-bound `ResolutionSet` 与 pure peer matcher；可注入 helper ledger 覆盖固定 READY、CAS ownership transfer、single START、有界 result 与 cleanup 状态。production spawner 明确 fail-closed；完整离线测试 436/436。B2a 仍缺把 Gate 与 helper 强制串联的唯一 coordinator、可取消 kernel 与 result/start proof binding，因此尚未 complete；
...
当前工作区已经实现 W09-B2a 的部分离线原语：AttemptGate 的 transport claim 由 caller UUID 独占并线性化为 `io_claimed`，helper handoff proof 与 DNS START proof 均只能使用一次；`ResolutionSet` 从 AttemptPermit 私有冻结绑定派生 host/port/policy，并对 canonical transcript 执行上述整组地址策略；helper lifecycle 仅通过 injected fake kernel 验证 READY、owner CAS、single START、bounded result 与 cleanup 状态。尚未实现把这些原语强制串成唯一生产调用的 coordinator，raw result 也尚未 echo/bind DNS START proof；没有 cancellable native kernel/helper executable/spawn shim、真实 kill/reap/FD liveness、实际 DNS、numeric socket/connect 或 `getpeername()` I/O。`FailClosedProductionHelperSpawner` 在这些证据存在前始终零进程 fail-closed，因此 W09-B2a、W09-B2 与 M5 均不得标记 complete。
...
**当前实现状态（2026-08-31）**：M0–M4、M5/W08、W09-A、W09-B0 与 W09-B1 已完成并进入远端 `main@543d390`；M5 仍在进行，当前未提交工作区推进了 W09-B2a 的 owner/address/helper primitives，W09-B2a 后续、W09-B2b–B4 及 M6–M9 尚未完成。新增 caller-owned `io_claimed`、terminal-guard/DNS START proof、内容寻址 IANA 地址策略、AttemptPermit-bound `ResolutionSet`、pure peer matcher，以及只接受 injected kernel 的 READY→transfer→single START helper ledger；完整离线测试 436/436、99 文件 Python 3.10 grammar、compileall 与 diff check 均通过。本机实际运行时仍为 Python 3.12.13，真实 Python 3.10 runtime suite 证据仍缺失。这些路径只使用 fake source/kernel，保持零真实环境凭据、零真实进程、零 client、零 DNS/socket/HTTP。production credential source、唯一 coordinator、cancellable native strict-spawn helper、result/start proof binding、真实 cancel/kill/reap/FD liveness、numeric connect、精确 HTTP/1.1 `send_once`、TLS/hostname、连接 peer/rebinding、redirect 禁止与全资源 I/O cleanup 仍不存在，因此 W09-B2a/W09-B2/M5 不得升级为 complete。GLM 仍没有 VerificationRecord，只派生 `experimental`；应用仍被有意冻结，也没有真实 Quartz/TCC、topology/选区/截图、NSPanel、Provider API、live smoke 或真实 macOS E2E。Phase 1 query 固定为空；`QueryPolicyKind.EXACT` 仍未启用。

`M1–M4`、M5/W08、W09-A、W09-B0 与 W09-B1 已经完成；下一步先收口 B2a coordinator、cancellation/result binding 与 cleanup-failure 合同，再完成 B2b native production resolver、numeric single-connect 与真实 peer/rebinding proof，然后依次完成 B3 exact Transport 与 B4 完整矩阵，随后 W10 离线故障矩阵也必须通过。首次真实 API 只能发生在 M6/W11，且需要另行明确授权、只用固定非敏感合成图、只调用一次并关闭自动 retry；真实用户截图只能在该 smoke 完成后的 M7/W12 接入。详细工作包、依赖和当前状态以 Implementation Plan 为准；Plan 不得弱化本 Spec 的约束。
